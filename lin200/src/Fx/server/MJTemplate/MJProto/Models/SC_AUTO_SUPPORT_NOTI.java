...
public class SC_AUTO_SUPPORT_NOTI extends Fx.server.MJTemplate.MJProto.MJAProtoMessage {
	
	public static SC_AUTO_SUPPORT_NOTI newInstance() {
		return new SC_AUTO_SUPPORT_NOTI();
	}
	//PSS 관련
	@Override
	public MJEProtoMessages getMessageType() {
		return MJEProtoMessages.SC_AUTO_SUPPORT_NOTI;
	}
	...
	private SC_AUTO_SUPPORT_NOTI() {
		mEnabled = false;
	}

	public SC_AUTO_SUPPORT_NOTI setEnabled(boolean enabled){
		mEnabled = enabled;
		return this;
	}
	@Override
	...
	public int getSerializedSize() {
		int size = 0;
		int sequence = 1;
		size += Fx.server.MJTemplate.MJProto.ProtoOutputStream.computeBoolSize(sequence++, mEnabled);
	...
	public void writeTo(Fx.server.MJTemplate.MJProto.ProtoOutputStream output) throws java.io.IOException {
		int sequence = 1;
		output.writeBool(sequence++, mEnabled);
	}
