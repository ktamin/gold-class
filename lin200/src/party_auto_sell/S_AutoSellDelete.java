...
public class S_AutoSellDelete extends ServerBasePacket {

	static synchronized public BasePacket clone(BasePacket bp, PcInstance pc, List<AutosellItem> list) {
		if (bp == null)
			bp = new S_AutoSellDelete(pc, list);
		else
			((S_AutoSellDelete) bp).toClone(pc, list);
		return bp;
	}

	public S_AutoSellDelete(PcInstance pc, List<AutosellItem> list) {
		toClone(pc, list);
	}
