...
public class ItemArmorBreak extends ItemInstance {
	...
	static synchronized public ItemInstance clone(ItemInstance item) {
		...
		item.setSkill(SkillDatabase.find(716));
		return item;
	}
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		...
		// 플러그인 확인.
		Object p = PluginController.init(SkillController.class, "toSkill", cha, cbp, 15, 6);
		if (p != null) {
			java.lang.System.out.print("11111 \n");
			return;
		}
		...
		if (!Util.isDistance(cha, target, 3)) {
			...
			if (SkillController.isFigure(cha, target, skill, true, false)) {
		    try {
