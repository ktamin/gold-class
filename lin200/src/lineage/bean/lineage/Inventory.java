...
public class Inventory {
	...
	public void toDrop(ItemInstance item, final long count, int x, int y, boolean packet) {
		if (!cha.isInvis() && isRemove(item, count, packet, false, false)) {
			// 로그용 변수.
			...
			// 드랍됫다는거 알리기.
			item.toDrop(cha);
			((PcInstance) cha).toCharacterSave2();
			...
			if (!Common.system_config_console && !(cha instanceof PcRobotInstance) && cha instanceof PcInstance) {
				...
				String log = String.format("[%s]\t [드랍]\t [캐릭터: %s]\t [아이템: %s]\t %s\t [좌표 %d, %d, %d]", timeString,
						cha.getName(), Util.getItemNameToString(item, count), Util.getMapName(cha), cha.getX(),
						cha.getY(), cha.getMap());
	...
	public void toPickup(object o, final long count) {
		...
		if (!cha.isInvis() && o instanceof ItemInstance && isAppend((ItemInstance) o, count, false)) {
			...
			if (item.getCount() == count) {
				...
				/*
				 * // 추가.
				 * item_new_objid = append(item, count).getObjectId();
				 * // 갯수 갱신 표현.
				 * item.setCount(item.getCount() - count);
				 * item.toSender(S_ObjectAdd.clone(BasePacketPooling.getPool(S_ObjectAdd.class),
				 * item, item), false);
				 */
			}
			// 파티원들에게 표현하기.
			if (Lineage.party_pickup_item_print && cha instanceof PcInstance) {
				...
				if (p != null) {
					...
					p.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), null,
							Lineage.CHATTING_MODE_PARTY, msg));
				}
			}
			//
			...
			if (!Common.system_config_console && !(cha instanceof PcRobotInstance) && cha instanceof PcInstance) {
				long time = System.currentTimeMillis();
				String timeString = Util.getLocaleString(time, true);
				String log = String.format("[%s]\t [줍기]\t [캐릭터: %s]\t [아이템: %s]\t %s\t [좌표 %d, %d, %d]", timeString,
						cha.getName(), Util.getItemNameToString(item, count), Util.getMapName(cha), cha.getX(),
						cha.getY(), cha.getMap());
	...
	public ItemInstance append(ItemInstance item, long count, Object... opt) {
		...
		if (temp != null) {
			...
			// ▼▼▼ [자동판매 트리거 1] 기존 아이템과 겹쳐질 때 발동 ▼▼▼
			if (cha instanceof PcInstance) {
				((PcInstance) cha).tryAutoSellOnPickup(temp);
			}
			// ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
			...
			if (type != null)
				Log.appendItem(cha, type, String.format("npc_name|%s", npc_name),
						...
						String.format("item_objid|%s", item_objid), String.format("target_name|%s", target_name),
						String.format("target_objid|%s", target_objid), String.format("count|%d", count));
		} else {
			if (item.getItem().isPiles()) {
				// 객체 생성.
				...
				for (int i = 0; i < count; ++i) {
					...
					if (type != null)
						Log.appendItem(cha, type, String.format("npc_name|%s", npc_name),
								String.format("npc_objid|%d", npc_objid), String.format("item_name|%s", item_name),
								String.format("item_objid|%s", item_objid),
								String.format("target_name|%s", target_name),
								String.format("target_objid|%s", target_objid),
	...
	public void append(ItemInstance item, boolean packet) {
		if (item != null && item.getCount() >= 0) {
			...
			// ▼▼▼ [자동판매 트리거 2] 인벤에 새로운 아이템이 들어올 때 발동 ▼▼▼
			if (cha instanceof PcInstance) {
				((PcInstance) cha).tryAutoSellOnPickup(item);
			}
			// ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
		}
	}
					...
					if (item instanceof Arrow) {
						...
						if (item.isEquipped())
							ChattingController.toChatting(cha,
									String.format("%s 소모. 다른 화살을 무장 하십시오.  ", item.getItem().getName()),
	...
	public boolean isAppend(ItemInstance item, long count, boolean shop) {
		...
		return isAppend(item.getItem(), count, item.getItem().isPiles() ? 1 : count);
	}
	...
	public double getMaxWeight() {
		...
		if (cha != null) {
			if (cha instanceof PcInstance) {
				max_weight = getClassMaxWeight(cha) + CharacterController.toStatStr(cha, "getMaxWeight")
						+ CharacterController.toStatCon(cha, "getMaxWeight") + cha.getItemWeight();
	...
	public ItemInstance find(String name, int enLev, int bress) {
		synchronized (list) {
			for (ItemInstance item : list) {
				if (item.getItem().getName().equalsIgnoreCase(name) && item.getBless() == bress
	...
	public boolean isRingOfPolymorphControl() {
		ItemInstance r1 = getSlot(Lineage.SLOT_RING_LEFT);
		ItemInstance r2 = getSlot(Lineage.SLOT_RING_RIGHT);
		if ((r1 != null && r1.getItem().getName().equalsIgnoreCase("변신 지배 반지"))
				|| (r2 != null && r2.getItem().getName().equalsIgnoreCase("변신 지배 반지"))) {
			return true;
		}
		return false;
	}
	 ...
	 * ItemInstance r1 = getSlot(Lineage.SLOT_RING_LEFT);
	 * ItemInstance r2 = getSlot(Lineage.SLOT_RING_RIGHT);
	 * return (r1 != null && r1 instanceof RingTeleportControl) || (r2 != null && r2
	 * instanceof RingTeleportControl);
	 * }
	 */
	/**
	 * 순간이동 조종반지 또는 순간이동 지배 반지 착용여부 리턴
	 */
	public boolean isRingOfTeleportControl() {
		ItemInstance r1 = getSlot(Lineage.SLOT_RING_LEFT);
		ItemInstance r2 = getSlot(Lineage.SLOT_RING_RIGHT);

		if ((r1 != null && r1.getItem().getName().equalsIgnoreCase("순간이동 지배 반지"))
				|| (r2 != null && r2.getItem().getName().equalsIgnoreCase("순간이동 지배 반지"))) {
			return true;
		}

		return (r1 != null && r1 instanceof RingTeleportControl)
				|| (r2 != null && r2 instanceof RingTeleportControl);
	}
	...
	public void checkAden(ItemInstance aden) {
		if (aden != null && aden.getItem() != null && (aden.getItem().getName().equalsIgnoreCase("아데나")
				|| aden.getItem().getNameId().equalsIgnoreCase("$4")) && aden.getCount() > Common.MAX_COUNT) {
			if (aden.getCount() - Common.MAX_COUNT >= Common.ONE_HUNDRED_MILLION) {
				...
				if (count > 0) {
					...
					if (i != null) {
						...
						// 알림.
						ChattingController.toChatting(cha,
								String.format("[20억 아데나 초과 소지] %s(%d) 획득.", i.getName(), count),
								Lineage.CHATTING_MODE_MESSAGE);
					}
				}
			}
	...
	public 클래스변경권 is클래스변경주문서(PcInstance pc, long objId) {
		if (this != null) {
			for (ItemInstance i : getList()) {
				if (i != null && i.getObjectId() == objId && i instanceof 클래스변경권) {
					return (클래스변경권) i;
				}
			}
		}
	...
	public boolean 활장착여부() {
		return getSlot(Lineage.SLOT_WEAPON) == null ? false
				: getSlot(Lineage.SLOT_WEAPON).getItem().getType2().equalsIgnoreCase("bow");
	}
