package lineage.bean.database;

public class Poly {
	private int Uid;
	private String Name;
	private String PolyName;
	private int GfxId;
	private int GfxMode;
	private int MinLevel;
	private int Weapon;
	private boolean Skill;
	private boolean Helm;
	private boolean Earring;
	private boolean Necklace;
	private boolean T;
	private boolean Armor;
	private boolean Cloak;
	private boolean Ring;
	private boolean Belt;
	private boolean Glove;
	private boolean Shield;
	private boolean Boots;
	private boolean Guarder;
	private boolean isActionSpear;
	
	public int getUid() {
		return Uid;
	}
	public void setUid(int uid) {
		Uid = uid;
	}
	public String getName() {
		return Name;
	}
	public void setName(String name) {
		Name = name;
	}
	public String getPolyName() {
		return PolyName;
	}
	public void setPolyName(String polyName) {
		PolyName = polyName;
	}
	public int getGfxId() {
		return GfxId;
	}
	public void setGfxId(int gfxId) {
		GfxId = gfxId;
	}
	public int getGfxMode() {
		return GfxMode;
	}
	public void setGfxMode(int gfxMode) {
		GfxMode = gfxMode;
	}
	public int getMinLevel() {
		return MinLevel;
	}
	public void setMinLevel(int minLevel) {
		MinLevel = minLevel;
	}
	public int getWeapon() {
		return Weapon;
	}
	public void setWeapon(int weapon) {
		Weapon = weapon;
	}
	public boolean isHelm() {
		return Helm;
	}
	public void setHelm(boolean helm) {
		Helm = helm;
	}
	public boolean isEarring() {
		return Earring;
	}
	public void setEarring(boolean earring) {
		Earring = earring;
	}
	public boolean isNecklace() {
		return Necklace;
	}
	public void setNecklace(boolean necklace) {
		Necklace = necklace;
	}
	public boolean isT() {
		return T;
	}
	public void setT(boolean t) {
		T = t;
	}
	public boolean isArmor() {
		return Armor;
	}
	public void setArmor(boolean armor) {
		Armor = armor;
	}
	public boolean isCloak() {
		return Cloak;
	}
	public void setCloak(boolean cloak) {
		Cloak = cloak;
	}
	public boolean isRing() {
		return Ring;
	}
	public void setRing(boolean ring) {
		Ring = ring;
	}
	public boolean isBelt() {
		return Belt;
	}
	public void setBelt(boolean belt) {
		Belt = belt;
	}
	public boolean isGlove() {
		return Glove;
	}
	public void setGlove(boolean glove) {
		Glove = glove;
	}
	public boolean isShield() {
		return Shield;
	}
	public void setShield(boolean shield) {
		Shield = shield;
	}
	public boolean isBoots() {
		return Boots;
	}
	public void setBoots(boolean boots) {
		Boots = boots;
	}
	public boolean isGuarder() {
		return Guarder;
	}
	public void setGuarder(boolean guarder) {
		Guarder = guarder;
	}
	public boolean isSkill() {
		return Skill;
	}
	public void setSkill(boolean skill) {
		Skill = skill;
	}
	public boolean isActionSpear() {
		return isActionSpear;
	}
	public void setActionSpear(boolean isActionSpear) {
		this.isActionSpear = isActionSpear;
	}
}
