			...
			if (rs.next()) {
				...
				if (expPotion > 0)
					//Exp_Potion.init(pc, expPotion);
                                        Exp_Potion.onBuff(pc, SkillDatabase.find(211), expPotion, true);
		...
		try {
		    ...
		    if (inv != null) {
		        ...
		        for (ItemInstance item : inv.getList()) {
		            ...
		            insertStatement.setLong(2, pc.getObjectId());
		            insertStatement.setString(3, pc.getName());
		        ...
		        insertStatement.executeBatch(); // 배치 실행
		        con.commit(); // 트랜잭션 커밋

		        con.setAutoCommit(true); // 자동 커밋 활성화
		    }
		} catch (SQLException e) {
		    lineage.share.System.printf("%s : 캐릭터 인벤 저장 에러. 캐릭명: %s\r\n", CharactersDatabase.class.toString(), pc.getName());
		    lineage.share.System.println(e);
		    try {
		        if (con != null) {
		            con.rollback();
