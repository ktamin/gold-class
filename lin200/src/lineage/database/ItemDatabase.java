...
import lineage.world.object.item.all_night.LevelupSupport2;
import lineage.world.object.item.all_night.NPC제거막대;
...
import lineage.world.object.item.all_night.huntgo2;
import lineage.world.object.item.all_night.notice;
import lineage.world.object.item.all_night.omanmaster;
import lineage.world.object.item.all_night.가더2차변경주문서;
import lineage.world.object.item.all_night.경험치구슬;
import lineage.world.object.item.all_night.경험치저장구슬;
import lineage.world.object.item.all_night.귀걸이2차변경주문서;
import lineage.world.object.item.all_night.귀걸이변경주문서;
...
import lineage.world.object.item.all_night.생명의나뭇잎;
import lineage.world.object.item.all_night.생명의마안교환서;
import lineage.world.object.item.all_night.셀프시전마법;
import lineage.world.object.item.all_night.아이템드랍확인막대;
import lineage.world.object.item.all_night.암석변경주문서;
import lineage.world.object.item.all_night.용갑옷변경주문서;
import lineage.world.object.item.all_night.용갑옷속성변경주문서;
import lineage.world.object.item.all_night.이뮨투함;
import lineage.world.object.item.all_night.인벤확인주문서;
import lineage.world.object.item.all_night.인첸트복구주문서;
import lineage.world.object.item.all_night.인첸트제거주문서;
import lineage.world.object.item.all_night.인형변경주문서;
import lineage.world.object.item.all_night.인형옵션부여주문서;
import lineage.world.object.item.all_night.인형진화주문서;
import lineage.world.object.item.all_night.자동칼질;
import lineage.world.object.item.all_night.장갑변경주문서;
import lineage.world.object.item.all_night.장비확인막대;
import lineage.world.object.item.all_night.최대HP증가물약;
import lineage.world.object.item.all_night.최대MP증가물약;
import lineage.world.object.item.all_night.캐릭터저장구슬;
...
import lineage.world.object.item.scroll.ScrollOf100ElementUpgrade;
import lineage.world.object.item.scroll.ScrollOfChangeElementalWeapon;
...
import lineage.world.object.item.scroll.ScrollOfRemoveElementalWeapon;
import lineage.world.object.item.scroll.ScrollPolymorph;
import lineage.world.object.item.scroll.ScrollPolymorph2;
...
import lineage.world.object.item.yadolan.ScrollOfdds;
import lineage.world.object.item.yadolan.ScrollOfdke;
import lineage.world.object.item.yadolan.ScrollOfdkn;
import lineage.world.object.item.yadolan.ScrollOfdp;
import lineage.world.object.item.yadolan.ScrollOfdp2;
import lineage.world.object.item.yadolan.ScrollOfdp3;
import lineage.world.object.item.yadolan.ScrollOfdp4;
import lineage.world.object.item.yadolan.ScrollOfdpa;
import lineage.world.object.item.yadolan.ScrollOfdpad;
import lineage.world.object.item.yadolan.ScrollOfdr;
import lineage.world.object.item.yadolan.ScrollOfds;
import lineage.world.object.item.yadolan.ScrollOfdv;
import lineage.world.object.item.yadolan.ScrollOfgd;
import lineage.world.object.item.yadolan.ScrollOfgd3;
...
import lineage.world.object.item.yadolan.무기변환;
import lineage.world.object.item.yadolan.미티어막대기;
import lineage.world.object.item.yadolan.방어구변환;
import lineage.world.object.item.yadolan.버프시간;
import lineage.world.object.item.yadolan.자동사냥시간초기화;
import lineage.world.object.item.yadolan.자동사냥시간초기화2;
import lineage.world.object.item.yadolan.자동사냥캐릭시간초기화;
import lineage.world.object.item.yadolan.자칼뱀파;
import lineage.world.object.item.yadolan.자칼선버;
import lineage.world.object.item.yadolan.자칼스턴;
...
public final class ItemDatabase {
	...
	static public ItemInstance newInstance(Item item) {
		...
		switch (item.getNameIdNumber()) {
			...
			case 1864: // 마법서 (위크니스) 아머 브레이크로 변경
			...
			case 2518: // 흑정령의 수정 (블라인드 하이딩)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 0)
						.clone(item);
			case 2519: // 흑정령의 수정 (인챈트 베놈)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 1)
						.clone(item);
			case 2520: // 흑정령의 수정 (쉐도우 아머)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 2)
						.clone(item);
			case 2521: // 흑정령의 수정 (브링 스톤)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 3)
						.clone(item);
			case 2522: // 흑정령의 수정 (무빙 악셀레이션)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 4)
						.clone(item);
			case 2523: // 흑정령의 수정 (버닝 스피릿츠)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 5)
						.clone(item);
			case 2524: // 흑정령의 수정 (다크 블라인드)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 6)
						.clone(item);
			case 2525: // 흑정령의 수정 (베놈 레지스트)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 13, 7)
						.clone(item);
			case 2526: // 흑정령의 수정 (더블 브레이크)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 0)
						.clone(item);
			case 2527: // 흑정령의 수정 (언케니 닷지)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 1)
						.clone(item);
			case 2528: // 흑정령의 수정 (쉐도우 팽)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 2)
						.clone(item);
			case 2529: // 흑정령의 수정 (파이널 번)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 3)
						.clone(item);
			case 2810: // 진화의 열매
				return EvolutionFruit.clone(getPool(EvolutionFruit.class)).clone(item);
			case 3172: // 흑정령의 수정 (드레스 마이티)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 4)
						.clone(item);
			case 3173: // 흑정령의 수정 (드레스 덱스터리티)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 5)
						.clone(item);
			case 3174: // 흑정령의 수정 (드레스 이베이젼)
				return ItemDarkSpiritCrystalInstance.clone(getPool(ItemDarkSpiritCrystalInstance.class), 14, 6)
						.clone(item);
			case 3303: // 말하는 섬 마을 귀환 주문서
				return ScrollReturnTalkingIslandVillage.clone(getPool(ScrollReturnTalkingIslandVillage.class))
						.clone(item);
			...
			case 5225: // 마법인형: 늑대인간
			case 5226: // 마법인형 : 버그베어
			case 5418: // 마법인형 : 크러스트시안
			case 7448: // 마법인형 : 에티
			case 5967: // 마법인형 : 목각
			case 5227: // 마법인형 : 서큐버스
			case 5417: // 마법인형 : 장로
			case 7511: // 마법인형 : 코카트리스
			case 10129: // 마법인형 : 눈사람
			case 5983: // 마법인형 : 인어
			case 5984: // 마법인형 : 라바 골렘
			case 20697: // 마법인형 : 자이언트
			case 5985: // 마법인형 : 흑장로
			case 12855: // 마법인형 : 서큐버스 퀸
			case 5968: // 마법인형 : 드레이크
			case 5969: // 마법인형 : 킹 버그베어
			case 5970: // 마법인형 : 다이아몬드 골렘
			case 20693: // 마법인형 : 리치
			case 20698: // 마법인형 : 사이클롭스
			case 5971: // 마법인형 : 나이트발드
			case 5972: // 마법인형 : 시어
			case 5973: // 마법인형 : 아이리스
			case 5974: // 마법인형 : 뱀파이어
			case 5975: // 마법인형 : 머미로드
			case 5976: // 마법인형 : 데몬
			case 5977: // 마법인형 : 데스나이트
			case 5978: // 마법인형 : 바란카
			case 5979: // 마법인형 : 타락
			case 5980: // 마법인형 : 바포메트
			case 5981: // 마법인형 : 얼음여왕
			...
			default:
				if (item.getType1().equalsIgnoreCase("weapon")) {
					...
					} else if (item.getType2().equalsIgnoreCase("edoryu")) {
	...
	static private ItemInstance newDefaultItem(Item item) {
		if (item.getType2().equalsIgnoreCase("bravery potion")) {
		...
		} else if (item.getType2().equalsIgnoreCase("polymorph")) {
			// 변신 주문서
			return ScrollPolymorph.clone(getPool(ScrollPolymorph.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("polymorph2")) {
			// 변신 주문서
			return ScrollPolymorph2.clone(getPool(ScrollPolymorph2.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("polymorph3")) {
			// 변신 주문서
		...
		} else if (item.getType2().equalsIgnoreCase("whkvyqhrrn")) {
			return 좌표복구주문서.clone(getPool(좌표복구주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dp_poly")) {
			return ScrollOfdp.clone(getPool(ScrollOfdp.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dp_poly2")) {
			return ScrollOfdp2.clone(getPool(ScrollOfdp2.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dp_poly3")) {
			return ScrollOfdp3.clone(getPool(ScrollOfdp3.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dp_poly4")) {
			return ScrollOfdp4.clone(getPool(ScrollOfdp4.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("gd_poly")) {
			return ScrollOfgd.clone(getPool(ScrollOfgd.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("gd_poly3")) {
			return ScrollOfgd3.clone(getPool(ScrollOfgd3.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("gd_poly4")) {
			return ScrollOfgd4.clone(getPool(ScrollOfgd4.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dr_poly")) {
			return ScrollOfdr.clone(getPool(ScrollOfdr.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("grts_poly")) {
			return ScrollOfgrts.clone(getPool(ScrollOfgrts.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dtn_poly")) {
			return ScrollOfdkn.clone(getPool(ScrollOfdkn.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dke_poly")) {
			return ScrollOfdke.clone(getPool(ScrollOfdke.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("ds_poly")) {
			return ScrollOfds.clone(getPool(ScrollOfds.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dv_poly")) {
			return ScrollOfdv.clone(getPool(ScrollOfdv.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dpa_poly")) {
			return ScrollOfdpa.clone(getPool(ScrollOfdpa.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dpad_poly")) {
			return ScrollOfdpad.clone(getPool(ScrollOfdpad.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("dds_poly")) {
			return ScrollOfdds.clone(getPool(ScrollOfdds.class)).clone(item);
			...
			return 버프시간.clone(getPool(버프시간.class)).clone(item);
			...
			return omanmaster.clone(getPool(notice.class)).clone(item);
			...
			// 생명의 나뭇잎
			return 생명의나뭇잎.clone(getPool(생명의나뭇잎.class)).clone(item);
		...
		} else if (item.getType2().equalsIgnoreCase("생명의 마안 교환서")) {
			// 생명의 마안 교환서
			return 생명의마안교환서.clone(getPool(생명의마안교환서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("무한 물약")) {
			// 무한 물약
			return HealingPotion.clone(getPool(HealingPotion.class)).clone(item);

		} else if (item.getType2().contains("경험치 물약")) {
			// 경험치 물약
		...
		} else if (item.getType2().equalsIgnoreCase("무기 변경 주문서")) {
			// 무기 변경 주문서
			return 무기변경주문서.clone(getPool(무기변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("투구 변경 주문서")) {
			// 투구 변경 주문서
			return 투구변경주문서.clone(getPool(투구변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("티셔츠 변경 주문서")) {
			// 티셔츠 변경 주문서
			return 티셔츠변경주문서.clone(getPool(티셔츠변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("장갑 변경 주문서")) {
			// 장갑 변경 주문서
			return 장갑변경주문서.clone(getPool(장갑변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("부츠 변경 주문서")) {
			// 부츠 변경 주문서
			return 부츠변경주문서.clone(getPool(부츠변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("목걸이 변경 주문서")) {
			// 목걸이 변경 주문서
			return 목걸이변경주문서.clone(getPool(목걸이변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("귀걸이 변경 주문서")) {
			// 귀걸이 변경 주문서
			return 귀걸이변경주문서.clone(getPool(귀걸이변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("반지 변경 주문서")) {
			// 반지 변경 주문서
			return 반지변경주문서.clone(getPool(반지변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("벨트 변경 주문서")) {
			// 벨트 변경 주문서
			return 벨트변경주문서.clone(getPool(벨트변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("마법 변경 주문서")) {
			// 무기 변경 주문서
			return 마법변경주문서.clone(getPool(마법변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("마안 변경 주문서")) {
			// 마안 변경 주문서
			return 마안변경주문서.clone(getPool(마안변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("변신 카드 변환")) {
			// 무기 변경 주문서
			return 변카변경주문서.clone(getPool(변카변경주문서.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("용갑옷 변경 주문서")) {
			// 용갑옷 변경 주문서
			return 용갑옷변경주문서.clone(getPool(용갑옷변경주문서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("용갑옷 속성 변경 주문서")) {
			// 용갑옷 속성 변경 주문서
		...
		} else if (item.getType2().equalsIgnoreCase("마물 변경 주문서")) {
			// 마물 변경 주문서
			return 마물변경주문서.clone(getPool(마물변경주문서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("방패 변경 주문서")) {
			// 2차 귀걸이 변경 주문서
			...
			// 최대 HP 증가 물약
			return 최대HP증가물약.clone(getPool(최대HP증가물약.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("최대 MP 증가 물약")) {
			// 최대 MP 증가 물약
			return 최대MP증가물약.clone(getPool(최대MP증가물약.class)).clone(item);
			...
			// 이뮨 투 함
			return 이뮨투함.clone(getPool(이뮨투함.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("자칼트리")) {
			return 자칼트리.clone(getPool(자칼트리.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("자칼뱀파")) {
			return 자칼뱀파.clone(getPool(자칼뱀파.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("자칼스턴")) {
			return 자칼스턴.clone(getPool(자칼스턴.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("자칼선버스트")) {
			return 자칼선버.clone(getPool(자칼선버.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("인벤 확인 주문서")) {
			// 인벤 확인 주문서
			return 인벤확인주문서.clone(getPool(인벤확인주문서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("영혼석")) {
			// 몬스터 영혼석
			return 몬스터영혼석.clone(getPool(몬스터영혼석.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("몬스터 드랍 확인 막대")) {
			// 몬스터 드랍 확인 막대
			return 몬스터드랍확인막대.clone(getPool(몬스터드랍확인막대.class)).clone(item);
			...
			// 장비 확인 막대
			return 장비확인막대.clone(getPool(장비확인막대.class)).clone(item);
			...
			// 클래스 변경권
			return 클래스변경권.clone(getPool(클래스변경권.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("인형 진화 주문서")) {
			// 인형 진화 주문서
			return 인형진화주문서.clone(getPool(인형진화주문서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("인형 변경 주문서")) {
			// 인형 변경 주문서
			return 인형변경주문서.clone(getPool(인형변경주문서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("인형 옵션 부여 주문서")) {
			// 인형 옵션 부여 주문서
			return 인형옵션부여주문서.clone(getPool(인형옵션부여주문서.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("캐릭터 저장 구슬")) {
			// 캐릭터 저장 구슬
			return 캐릭터저장구슬.clone(getPool(캐릭터저장구슬.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("경험치 저장 구슬")) {
			// 경험치 저장 구슬
			return 경험치저장구슬.clone(getPool(경험치저장구슬.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("경험치 구슬")) {
			// 경험치 구슬
			return 경험치구슬.clone(getPool(경험치구슬.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("인첸트 제거 주문서")) {
			// 인첸트 제거 주문서
			return 인첸트제거주문서.clone(getPool(인첸트제거주문서.class)).clone(item);
			...
			return 미티어막대기.clone(getPool(미티어막대기.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("속성 100% 주문서")) {
			// 속성 부여 100%
			return ScrollOf100ElementUpgrade.clone(getPool(ScrollOf100ElementUpgrade.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("levelup_support2")) {
			// 레벨 업
			return LevelupSupport2.clone(getPool(LevelupSupport2.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("속성 제거 주문서")) {
			// 무기 속성 제거 주문서
			return ScrollOfRemoveElementalWeapon.clone(getPool(ScrollOfRemoveElementalWeapon.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("속성 변경 주문서")) {
			// 무기 속성 제거 주문서
			return ScrollOfChangeElementalWeapon.clone(getPool(ScrollOfChangeElementalWeapon.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("혈맹 버프 물약")) {
			// 혈맹 버프 물약
			return 혈맹버프물약.clone(getPool(혈맹버프물약.class)).clone(item);

		} else if (item.getType2().equalsIgnoreCase("기란 마을 귀환 부적")) {
			// 기란 마을 귀환 부적
			return ScrollLabeledVerrYedHorae.clone(getPool(ScrollLabeledVerrYedHorae.class)).clone(item);
			...
			return 자동사냥시간초기화2.clone(getPool(자동사냥시간초기화2.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("자동사냥 캐릭 시간 초기화")) {
			return 자동사냥캐릭시간초기화.clone(getPool(자동사냥캐릭시간초기화.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("클변 무기 변환")) {
			return 무기변환.clone(getPool(무기변환.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("클변 방어구 변환")) {
			return 방어구변환.clone(getPool(방어구변환.class)).clone(item);
		} else if (item.getType2().equalsIgnoreCase("다엘 변경권")) {
			return 다엘변경권.clone(getPool(다엘변경권.class)).clone(item);
