...
public final class TimeDungeonDatabase {
	...
	static public void resetGiranDungeonTime() {
		...
		if (Lineage.dungeon_inti_time_message)
	...
	static public void resetAutoTime() {
		...
		for (PcInstance pc : World.getPcList())
			...
			World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), String.format("[알림] 자동사냥 초기화 사용횟수가 초기화 되었습니다.")));
	}
	static public void resetGiranDungeonScrollCount() {
		...
		if (Lineage.is_giran_dungeon_scroll_message)
			World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), String.format("[알림] 기란감옥 초기화 주문서 사용횟수가 초기화 되었습니다.")));
	}
