...
public class EnchantLostItemDatabase {
	...
	static public void init(Connection con) {
		...
		ResultSet rs = null;
		try {
			// [수정] 4대 속성 컬럼도 읽어오도록 수정 (SELECT * 이므로 자동 포함되지만 명시적으로 읽는 부분 추가)
			...
			while (rs.next()) {
				...
				// [추가] DB에서 4대 속성 불러오기
				el.setEnFire(rs.getInt("en_fire"));
				el.setEnWater(rs.getInt("en_water"));
				el.setEnWind(rs.getInt("en_wind"));
				el.setEnEarth(rs.getInt("en_earth"));
	...
	static public void append(object o, ItemInstance i, ItemInstance scroll) {
		if (o != null && i != null && i.getItem() != null && scroll != null && scroll.getItem() != null) {
			...
			synchronized (list) {
				...
				// [추가] 파괴된 아이템(i)에서 속성 정보 가져와서 세팅
				el.setEnFire(i.getEnFire());
				el.setEnWater(i.getEnWater());
				el.setEnWind(i.getEnWind());
				el.setEnEarth(i.getEnEarth());
	...
	static public void insertDB(EnchantLostItem el) {
		...
		try {
			...
			try {
				// [수정] 쿼리문에 4대 속성 컬럼 추가
				st = con.prepareStatement("INSERT INTO enchant_lost_item SET 캐릭터_objId=?, 캐릭터=?, 아이템_objId=?, 아이템=?, 인첸트=?, 축복=?, 수량=?, 잃은시간=?, 주문서=?, 주문서_축복=?, 지급여부=?, en_fire=?, en_water=?, en_wind=?, en_earth=?");
				...
				// [추가] 4대 속성 값 세팅
				st.setInt(12, el.getEnFire());
				st.setInt(13, el.getEnWater());
				st.setInt(14, el.getEnWind());
				st.setInt(15, el.getEnEarth());
		...
		if (lastSaveTime < time) {
			...
			try {
				...
				for (EnchantLostItem el : getList()) {
					if (!el.is지급여부()) {
						try {
							// [수정] save() 메서드에서도 4대 속성 저장하도록 쿼리 수정
							st = con.prepareStatement("INSERT INTO enchant_lost_item SET 캐릭터_objId=?, 캐릭터=?, 아이템_objId=?, 아이템=?, 인첸트=?, 축복=?, 수량=?, 잃은시간=?, 주문서=?, 주문서_축복=?, 지급여부=?, en_fire=?, en_water=?, en_wind=?, en_earth=?");
							st.setLong(1, el.getCha_objId());
							st.setString(2, el.getCha_name());
							st.setLong(3, el.getItem_objId());
