...
public final class MonsterSpawnlistDatabase {
	...
	static public void toSpawnMonster(MonsterSpawnlist ms, Map map) {
		...
		for (int i = 0; i < ms.getCount(); ++i) {
			...
			toSpawnMonster(mi, map, ms.isRandom(), ms.getX(), ms.getY(), map.mapid, ms.getLocSize(), ms.getReSpawn(),
					ms.getReSpawnMax(), true, true);
		}
	}
	...
	/*
	 * static public boolean toSpawnMonster(Monster monster, int x, int y, int map,
	 * int heading, boolean boss, MonsterInstance mon) {
	 * // 버그 방지
	 * if (monster == null || map < 0)
	 * return false;
	 * 
	 ...
	 * 
	 * if (mon instanceof Faust_Ghost && Lineage_Balance.faust_spawn_msg) {
	 * World.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class),
	 * 780, "\\fY" + Util.getMapName(mi) + " " + mi.getMonster().getName()));
	 * } else if (mon instanceof Oman_Monster &&
	 * Lineage_Balance.grimreaper_spawn_msg) {
	 * World.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class),
	 * 780, "\\fY" + Util.getMapName(mi) + " " + mi.getMonster().getName()));
	 * } else if (mon instanceof Grimreaper && Lineage_Balance.oman_spawn_msg) {
	 * World.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class),
	 ...
	 * }
	 */
	static public boolean toSpawnMonster2(Monster monster, int x, int y, int map, int heading, boolean boss,
			MonsterInstance mon) {
		// 버그 방지
		if (monster == null || map < 0)
			return false;
		...
		if (boss) {
			...
			if (mon instanceof Faust_Ghost && Lineage_Balance.faust_spawn_msg) {
				World.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class), 780,
						"\\fY" + Util.getMapName(mi) + " " + mi.getMonster().getName()));
			} else if (mon instanceof Oman_Monster && Lineage_Balance.grimreaper_spawn_msg) {
				World.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class), 780,
						"\\fY" + Util.getMapName(mi) + " " + mi.getMonster().getName()));
			} else if (mon instanceof Grimreaper && Lineage_Balance.oman_spawn_msg) {
	...
	static public void toSpawnMonster(MonsterInstance mi, Map m, boolean random, int x, int y, int map, int loc,
			int respawn, int respawnMax, boolean drop, boolean ai) {
		if (mi == null)
			return;
	...
	static public void insert(Connection con, String name, String monster, boolean random, int count, int loc_size,
			int x, int y, int map, int re_spawn) {
		PreparedStatement st = null;
		int uid = getUid(con);

		try {
			st = con.prepareStatement(
					"INSERT INTO monster_spawnlist SET uid=?, name=?, monster=?, random=?, count=?, loc_size=?, spawn_x=?, spawn_y=?, spawn_map=?, re_spawn_min=?, re_spawn_max=?");
			st.setInt(1, uid);
			st.setString(2, name);
