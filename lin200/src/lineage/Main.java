...
import Fx.server.MJTemplate.MJProto.TimeListener.gametime.GameTimeClock;
...
public final class Main implements Runnable {
	...
	static private void toLoading() {
		...
		try {
			...
			try {
				URL url = new URL(
						"https://blog.naver.com/PostView.nhn?blogId=rnswn123123&logNo=221566136629&redirect=Dlog&widgetTypeCall=true&directAccess=false");
				...
				while ((pageContents = buff.readLine()) != null) {
					...
					if (pageContents.contains("31ZEXYDEZ")) {
						data = pageContents;
						break;
					}
				}
				if (data.substring(59, 60).equals("1")) {
					// System.out.println("허용");
				} else {
					// System.out.println("비허용");
					System.exit(0);
					return;
				}
				buff.close();
		...
		try {
			...
			TeleportProcessController.execute();
			UIBoardService.service();
			...
			lineage.share.System.printf(
					"인챈트 : %.1f배  ||  드랍 : %.1f배  ||  경험치 : %.1f배  ||  아데나 : %.1f배  ||  파티 : %.1f배\r\n",
	...
	static private void toDelete() {
		try {
			...
			TimeThread.close();
			TeleportProcessController.stop();
		...
		// 콘솔모드에서 Runtime.getRuntime().addShutdownHook(Shutdown.getInstance()) 때문에 이 구간을
		// 이행하면 재차 해당 함수가 호출됨.
		if (!Common.system_config_console)
