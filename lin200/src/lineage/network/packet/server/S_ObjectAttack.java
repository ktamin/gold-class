...
public class S_ObjectAttack extends ServerBasePacket {
	...
	static synchronized public BasePacket clone(BasePacket bp, object me, object target, int action, int dmg, int effect, boolean bow, boolean visual, int x, int y) {
		if (bp == null)
	...
	public void clone(object me, object target, int action, int dmg, int effect, boolean bow, boolean visual, int x, int y) {	
		...
		if (me instanceof PcInstance && (action == Lineage.GFX_MODE_SPELL_DIRECTION || action == Lineage.GFX_MODE_SPELL_NO_DIRECTION)) {
			PcInstance pc = (PcInstance) me;
		...
		if (bow) {
			if (me.getInventory().findArrow() != null) {
				if (effect > 0)
					toBow(me, target, effect, visual, x, y);
				else
					toBow(me, target, 66, visual, x, y);
			} else {
				toBow(me, target, effect, visual, x, y);
			}
		} else {
