...
import lineage.network.packet.server.S_EXTENDED_PROTOBUF;
...
public final class LineageClient {
	...
	public String getAccountIp() {
		// TODO 유저 실제 아이피 받도록 추가
		if (!account_ip.equalsIgnoreCase(getRealIp())) {
			return getRealIp();
		}
		return account_ip;
	}
	...
	public void setRealIp(String value) {
		_realIp = value;
	}
	...
	public void toPacket(byte[] data) {
		...
		synchronized (sync_client) {
			...
			} else if(op == Opcodes.C_EXTENDED_PROTOBUF) {
				MJEProtoMessages.existsProto(this, data);
	...
	public void sendProto(ProtoOutputStream stream, boolean isClear) {
		...
		toSender(S_EXTENDED_PROTOBUF.clone(BasePacketPooling.getPool(S_EXTENDED_PROTOBUF.class), stream.getProtoBytes()));
		if (isClear)
			stream.dispose();
	}

	public void sendProto(ProtoOutputStream stream) {
		sendProto(stream, true);
	}

	public void sendProto(MJIProtoMessage message, int messageId, boolean isClear) {
		if (message.isInitialized()) {
			sendProto(message.writeTo(MJEProtoMessages.fromInt(messageId)), isClear);
			if (isClear)
				message.dispose();
		} else {
			MJEProtoMessages.printNotInitialized(getPc() == null ? getAccountIp() : getPc().getName(), messageId, message.getInitializeBit());
		}
	}

	public void sendProto(MJIProtoMessage message, MJEProtoMessages messageId) {
		sendProto(message, messageId.toInt(), true);
	}

	public void sendProto(MJIProtoMessage message, MJEProtoMessages messageId, boolean isClear) {
		sendProto(message, messageId.toInt(), isClear);
	}

	public void sendProto(MJIProtoMessage message, int messageId) {
		sendProto(message, messageId, true);
	}
