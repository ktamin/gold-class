...
public final class CharacterController {
	        ...
	        try {
	            if (o != null && !o.isDead() && !o.isWorldDelete()) {
	                ...
	                if (o instanceof Character) {
	                    ...
	                    if (cha instanceof PcInstance && inv != null) {
	                        boolean isWeightOkay = inv.isWeightPercent(50);

	                        if (!isWeightOkay) {
	                            boolean isInn = InnController.isInnMap(cha);
	                            boolean isBuffed = cha.isBuffExoticVitalize() || cha.isBuffAdditionalFire();

	                            // 여관이나 특정 버프가 없으면 회복 차단
	                            if (!isInn && !isBuffed) {
	                                tic_hp = 0;
	                                tic_mp = 0;

	                                // 예외적으로 요정망토 착용 시 HP 회복 허용
	                                ItemInstance cloak = inv.getSlot(Lineage.SLOT_CLOAK);
	                                if (cloak instanceof ElvenCloak)
	                                    tic_hp = 1;
	                            }
	                        }
	                    }
	            ...
	            String name = (o instanceof Character) ? ((Character)o).getName() : "알 수 없음";
	            lineage.share.System.println("자연회복 처리 오류 발생 - 이름: " + name + ", 객체: " + o);
		...
		if (HpMp) { // hp
			...
			if (cha instanceof PcInstance) {
				...
				case Lineage.LINEAGE_CLASS_WIZARD: // 법사
					...
					break;
				case Lineage.LINEAGE_CLASS_DARKELF: // 기사
					if (wis < 10)
						HPMP = Util.random(0, 2);
					else if (wis < 15)
						HPMP = Util.random(1, 2);
	...
	static public void toResetStat(PcInstance pc, int classType) {
		...
		case Lineage.LINEAGE_CLASS_WIZARD: // 마법사
			...
			break;
		case Lineage.LINEAGE_CLASS_DARKELF: // 다크엘프
			pc.setStr(Lineage.darkelf_stat_str);
			pc.setDex(Lineage.darkelf_stat_dex);
			pc.setCon(Lineage.darkelf_stat_con);
			pc.setInt(Lineage.darkelf_stat_int);
			pc.setWis(Lineage.darkelf_stat_wis);
			pc.setCha(Lineage.darkelf_stat_cha);
			break;	
		}
	...
	static public int toStatDex(Character cha, String type) {
		...
		if (type.equalsIgnoreCase("isHitFigure")) {
			...
			case Lineage.LINEAGE_CLASS_DARKELF:
				return (int) Math.round(hit * Lineage_Balance.darkelf_bow_hit_figure);	
			default:
				return hit;
			}
		}
