...
public class 기감컨트롤러 {
    ...
    public  static long    prisonEndTime;
    ...
    public static void init() {
        TimeLine.start("기감 컨트롤러..");
        calendar = Calendar.getInstance();
        isOpen = false;
//		prisonEndTime = Long.MAX_VALUE;
        prisonEndTime = 0L;
        lastRemainSecSent = -1;
        ...
        if (ALWAYS_OPEN) {
            ...
            isOpen = true;
            prisonEndTime = Long.MAX_VALUE; // 의미 없음이지만 닫히지 않도록 매우 크게
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.prison_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.prison_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
        ...
        // 열려 있으면 주기 갱신/종료 처리
        if (isOpen) {
            long diffMs   = prisonEndTime - nowMs;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        prisonEndTime = nowMs + (1000L * Lineage.prison_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg   = "\\fY      ***** 기란 감옥으로 가는길이 열렸습니다. *****";
            toastTitle = "★ 기란 감옥 입장 가능 ★";
            toastDesc  = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg   = "\\fY      ***** 기란 감옥으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 기란 감옥 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = prisonEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
