...
public class 테베사막컨트롤러 {
    // 상태
    private static Calendar calendar;
    public  static boolean isOpen;
    public  static long    desertEndTime;
    ...
    public static void init() {
        ...
        if (ALWAYS_OPEN) {
            ...
            isOpen = true;
            desertEndTime = Long.MAX_VALUE; // 의미 없음이지만 닫히지 않도록 매우 크게
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.desert_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.desert_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
        ...
        // 열려 있으면 주기 갱신/종료 처리
        if (isOpen) {
            long diffMs   = desertEndTime - nowMs;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        desertEndTime = nowMs + (1000L * Lineage.desert_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = desertEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
