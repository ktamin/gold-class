...
public class 고라스컨트롤러 {
    // 상태
    private static Calendar calendar;
    public  static boolean isOpen;
    public  static long    gorasEndTime;
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.goras_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.goras_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
        ...
        // 열려 있으면 주기 갱신/종료 처리
        if (isOpen) {
            long diffMs   = gorasEndTime - nowMs;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        gorasEndTime = nowMs + (1000L * Lineage.goras_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg   = "\\fY      ***** 고라스 던전으로 가는길이 열렸습니다. *****";
            toastTitle = "★ 고라스 던전 입장 가능 ★";
            toastDesc  = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg   = "\\fY      ***** 고라스 던전으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 고라스 던전 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        int remainSec = 0;
        if (isOpen) {
            long diff = gorasEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
