...
public final class TeamBattleController {
    ...
    static private boolean joinEnd;

    static public void init() {
        TimeLine.start("TeamBattleController..");
        calendar = Calendar.getInstance();
        joinList = new ArrayList<>();
        polyList = new HashMap<>();
        line = new ArrayList<>();
        TimeLine.end();
    }
        ...
        for (TeamBattleTime tempTime : Lineage.team_battle_time) {
            ...
            if (tempTime.getHour() == hour && tempTime.getMin() - Lineage.team_battle_world_message == min && sec == 0) {
                ...
                World.toSender(S_ObjectChatting.clone(
                        ...
                        String.format("[알림] %d분 후 팀대전이 시작됩니다.", Lineage.team_battle_world_message)
                ));
            }
                ...
                for (PcInstance pc : getJoinList()) {
                    if (!pc.isWorldDelete()) {
                        ChattingController.toChatting(
                                pc,
                                String.format("\\fR[알림] %d초 후 팀대전이 시작됩니다.", 60 - sec),
            ...
            // 승패 체크
            if (startTeamBattle) {
                if (A_TeamScore < 1) endTeamBattle(B_Team.getUid());
                else if (B_TeamScore < 1) endTeamBattle(A_Team.getUid());
            }
        }
    }
