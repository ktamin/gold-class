...
//		
//		//야도란 월드보스
//		if (cha instanceof PcInstance && target instanceof PcInstance && cha.getMap() == 1400 ) {
//			ChattingController.toChatting(cha, "월드보스에서는 pk 불가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
//			return 0;
//		}
		...
		if (dmg <= 0) {
			...
			// 브레이브 멘탈 1.8배
			if (!bow && cha.isBuffBraveMental() && Util.random(1, 100) <= 30)
				dmg *= 1.8;
	
			//바운스어택
			if (cha.getInventory().find("바운스 어택") != null && (cha.getClassType() == Lineage.LINEAGE_CLASS_KNIGHT || cha.getClassType() == Lineage.LINEAGE_CLASS_ELF) && Util.random(1, 100) <= 20 && !bow  ) {
					dmg *= 1.25;
...
//				dmg += DmgElement(weapon, target);
				...
				if (target instanceof MonsterInstance) {
					...
					if (mon.getMonster().isUndead()) {
						...
						case 17: // 미스릴
						case 22: // 오리하루콘
							dmg += Util.random(1, 20);
							dmg += !bow && weapon.isBuffHolyWeapon() ? Util.random(1, 5) : 0;
							break;
						}
					}
				}
			...
			if (is) {
				...
				if (Lineage.pvp_print_message) {
					...
					for (PcInstance p : World.getPcList()) {
						ChattingController.toChatting(p, String.format(	Lineage.war_ment, local , pc.getName(), use.getName()), Lineage.CHATTING_MODE_MESSAGE);
					SC_TOAST_NOTI.newInstance().setMessage(String.format("\\f3%s님이 %s님에게 %s", use.getName(), pc.getName(), msg)).setToastType(ToastType.Mockery).send(p);
...
*/	
	...
	/**
	 * 아이템 속성 옵션을 캐릭터에 누적/해제
	 * 불(근거리), 바람(원거리), 물(마법크리확률), 땅(리덕션)
	 * 아이템 기본 속성 + 속성부여(enXXX) 둘 다 반영
	 */
	public static void applyElementOption(ItemInstance item, Character cha, boolean equipped) {
	    int sign = equipped ? 1 : -1;

	    // 불: 근거리 데미지
	    int fireLevel = item.getEnFire();  // 공격 속성만
	    if (fireLevel > 0) {
	        int value = getFireOptionValue(fireLevel) * sign;
	        cha.setDynamicAddDmg(cha.getDynamicAddDmg() + value);
	    }

	    // 바람: 원거리 데미지
	    int windLevel = item.getEnWind();
	    if (windLevel > 0) {
	        int value = getWindOptionValue(windLevel) * sign;
	        cha.setDynamicAddDmgBow(cha.getDynamicAddDmgBow() + value);
	    }

	 // 물: 스펠파워
	    int waterLevel = item.getEnWater();
	    if (waterLevel > 0) {
	        int value = getWaterOptionValue(waterLevel) * sign;
	        cha.setDynamicSp(cha.getDynamicSp() + value);
	    }

	    // 땅: 리덕션
	    int earthLevel = item.getEnEarth();
	    if (earthLevel > 0) {
	        int value = getEarthOptionValue(earthLevel) * sign;
	        cha.setDynamicReduction(cha.getDynamicReduction() + value);
	    }
	}

	
	// 단계별 효과값 반환 함수도 아래처럼 깔끔하게!
	private static int getFireOptionValue(int level) {
	    switch (level) {
	        case 1: return 1;
	        case 2: return 3;
	        case 3: return 7;
	...
	private static int getWindOptionValue(int level) {
	    switch (level) {
	        case 1: return 1;
	        case 2: return 3;
	        case 3: return 7;
	...
	private static int getWaterOptionValue(int level) {
	    switch (level) {
	        case 1: return 1;
	        case 2: return 3;
	        case 3: return 7;
	...
	private static int getEarthOptionValue(int level) {
	    switch (level) {
	        case 1: return 1;
	        case 2: return 3;
	        case 3: return 7;
	...
	static private boolean isHitFigure(Character cha, object target, boolean bow, ItemInstance weapon) {
		...
		if (target instanceof PcInstance) {
			...
			case Lineage.LINEAGE_CLASS_DARKELF:
				if (bow)
					tempProbability *= Lineage_Balance.pc_bow_hit_ac_darkelf_percent;
				else
					tempProbability *= Lineage_Balance.pc_hit_ac_darkelf_percent;
				break;	
			}
		}
		...
		if (bow) {
			...
			if (cha instanceof MonsterInstance){
				...
				switch (target.getClassType()) {
					...
					break;
					case Lineage.LINEAGE_CLASS_KNIGHT:
						tempProbability *= Lineage_Balance.pc_hit_rate2;
					break;
					case Lineage.LINEAGE_CLASS_ELF:
						tempProbability *= Lineage_Balance.pc_hit_rate3;
						break;
					case Lineage.LINEAGE_CLASS_WIZARD:
						tempProbability *= Lineage_Balance.pc_hit_rate4;
						break;
					case Lineage.LINEAGE_CLASS_DARKELF:
						tempProbability *= Lineage_Balance.pc_hit_rate5;
						break;	
				}
			}
		}
...
//			if (mon.getLevel() <= 71 && mon.getLevel() >= 73 && !mon.isBoss()){
//				tempProbability -= 5;
//			}
//			if (mon.getLevel() <= 74 && mon.getLevel() >= 76 && !mon.isBoss()){
//				tempProbability -= 10;
//			}
//			if (mon.getLevel() <= 77 && mon.getLevel() >= 80 && !mon.isBoss()){
//				tempProbability -= 15;
//			}
//			if (mon.getLevel() <= 80 && mon.getLevel() >= 84 && !mon.isBoss()){
//				tempProbability -= 17;
//			}
//			if (mon.getLevel() <= 85 && mon.getLevel() >= 90 && !mon.isBoss()){
//				tempProbability -= 20;
//			}
//		}
		// 1레벨 차이당 1%감소
		...
		if(target.getGm() > 0){
			ChattingController.toChatting(target, String.format("%f", probability), Lineage.CHATTING_MODE_MESSAGE);
		}
		boolean result = Util.random(1, 100) <= Math.round(probability);
	...
	static public boolean 불멸의가호시스템(Character cha, PcInstance use, Kingdom k) {
	    // PvP 전용 모드이고 공격자가 PC가 아니면 패스
	    if (Lineage.is_immortality_pvp && !(cha instanceof PcInstance)) {
	        return true;
	    }

	    try {
	        // 시스템 꺼져 있거나, 객체가 없거나, 인벤토리 없으면 패스
	        if (!Lineage.is_immortality
	         || cha == null || use == null
	         || cha.getInventory() == null
	         || use.getInventory() == null) {
	            return true;
	        }
	        // 공성전 중 혈맹원 사망 시 패스
	        if (k != null && k.isWar()) {
	        ...
	        // 1) 우선 “고급 불멸의 가호” 체크
	        ItemInstance premInst = iv.find(Lineage.immortality_premium_item_name);
	        ItemInstance normInst = iv.find(Lineage.immortality_item_name);
	        boolean hasPremium = premInst != null && premInst.getCount() > 0;
	        boolean hasNormal  = normInst != null && normInst.getCount()  > 0;
	        ...
	        // 2) 실제 소모할 인스턴스 선택 후 차감
	        ItemInstance toUse = hasPremium ? premInst : normInst;
	        iv.count(toUse, toUse.getCount() - 1, true);
	        ...
	        // 4) 킬러에게 “타버린” 보상 지급
	        if (cha instanceof PcInstance) {
	        PcInstance killer = (PcInstance) cha;
	        String burnItem = hasPremium
	            ? Lineage.immortality_premium_burn_item_name
	            : Lineage.immortality_kill_item_name;
	        lineage.bean.database.Item tpl = ItemDatabase.find(burnItem);
	        if (tpl != null && killer != null && !killer.isWorldDelete()) {
	            ItemInstance reward = ItemDatabase.newInstance(tpl);
	            reward.setObjectId(ServerDatabase.nextItemObjId());
	            reward.setCount(1);
	            reward.setBless(1);
	            reward.setDefinite(true);
	            killer.toGiveItem(null, reward, 1);

	            ChattingController.toChatting(killer,
	                String.format("[불멸의 가호 보상] %s 1개 획득", burnItem),
	                Lineage.CHATTING_MODE_MESSAGE
	            );
	        }
	        }
	        ...
	        if (Lineage.is_immortality_kill_item) {
	            // ← 기존 보상 드랍/인벤토리 추가 코드를 이 자리에 그대로 옮겨 넣으세요.
	        }
