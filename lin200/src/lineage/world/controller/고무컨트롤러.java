...
public class 고무컨트롤러 {
    ...
    public  static long    gomuEndTime;
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.gomu_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.gomu_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
    ...
    private static void open(long nowMs) {
        isOpen = true;
        gomuEndTime = nowMs + (1000L * Lineage.gomu_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg   = "\\fY      ***** 고대 거인의 무덤으로 가는길이 열렸습니다. *****";
            toastTitle = "★ 고대 거인의 무덤 입장 가능 ★";
            toastDesc  = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg   = "\\fY      ***** 고대 거인의 무덤으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 고대 거인의 무덤 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = gomuEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
