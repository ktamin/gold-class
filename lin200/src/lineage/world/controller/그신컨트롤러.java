...
public class 그신컨트롤러 {
    ...
    public  static long    shadowEndTime;
...
//		shadowEndTime = Long.MAX_VALUE;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        shadowEndTime = nowMs + (1000L * Lineage.shadow_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void pushTimerTo(PcInstance pc) {
        int remainSec = 0;
        if (isOpen) {
            long diff = shadowEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
