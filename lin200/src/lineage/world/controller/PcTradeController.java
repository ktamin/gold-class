...
/*			ItemInstance item = o.getInventory().find(ItemDatabase.find("버프 물약30일"));
                                        if (item == null) {
						ChattingController.toChatting(o, "버프 물약30일을 소지하지 않고 있습니다.", Lineage.CHATTING_MODE_MESSAGE);
						return;
					}
			if (!checkInfo(pc))
				return;
...
//	        // 여러 인형 중 하나라도 있으면 통과
//	        ItemInstance doll = o.getInventory().find(ItemDatabase.find("마법인형: 기사"));
//	        if (doll == null) doll = o.getInventory().find(ItemDatabase.find("마법인형: 마법사"));
//	        if (doll == null) doll = o.getInventory().find(ItemDatabase.find("마법인형: 요정"));
//	        if (doll == null) doll = o.getInventory().find(ItemDatabase.find("마법인형: 군주"));
//
//	        if (doll == null) {
//	            ChattingController.toChatting(o, "클래스 인형을 소지 하고 있지 않습니다.", Lineage.CHATTING_MODE_MESSAGE);
//	            return;
//	        }
...
*/	        	        
			...
			int price = Integer.valueOf(st.nextToken());
			long itemCount = Long.valueOf(st.nextToken());
			String subject = String.format("%,d억 아데나 팝니다.", itemCount);
	...
	static public boolean insertItemFinal(PcInstance pc, ItemInstance item, long count) {
		...
		if (Lineage.is_pc_trade_sell_only_aden) {
			// 아데나 등록 못하도록 처리
			if (!item.getItem().getName().equals("아데나")) {
				pc.setPcTrade(null);
