...
public class 드워프컨트롤러 {
    // 상태
    private static Calendar calendar;
    public static boolean isOpen;
    public static long dwarfEndTime;
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.dwarf_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.dwarf_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
        ...
        // 열려 있으면 주기 갱신/종료 처리
        if (isOpen) {
            long diffMs = dwarfEndTime - nowMs;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        dwarfEndTime = nowMs + (1000L * Lineage.dwarf_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg = "\\fY      ***** 드워프 광산으로 가는길이 열렸습니다. *****";
            toastTitle = "★ 드워프 광산 입장 가능 ★";
            toastDesc = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg = "\\fY      ***** 드워프 광산으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 드워프 광산 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        int remainSec = 0;
        if (isOpen) {
            long diff = dwarfEndTime - System.currentTimeMillis();
            if (diff < 0)
                diff = 0;
