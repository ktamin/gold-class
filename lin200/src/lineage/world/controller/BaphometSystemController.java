...
public final class BaphometSystemController {
	...
	public static void setBaphomet(PcInstance pc) {
		 ...
		 * // 10000 ~ 19999
		 * if (pc.getLawful() >= 75536 && pc.getLawful() <= 85535 && (!pc.isBaphomet()
		 * || pc.getBaphometLevel() != 1)) {
		 * if (pc.isBaphomet())
		 * removeBaphomet(pc);
		 * pc.setBaphomet(true);
		 * pc.setBaphometLevel(1);
		 * appendBaphomet(pc);
		 * pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.
		 * class), pc, 10881), true);
		 * ChattingController.toChatting(pc, "\\fS라우풀 1단계: AC-3, 마법방어+1",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * // 20000 ~ 29999
		 * if (pc.getLawful() >= 85536 && pc.getLawful() <= 95535 && (!pc.isBaphomet()
		 * || pc.getBaphometLevel() != 2)) {
		 * if (pc.isBaphomet())
		 * removeBaphomet(pc);
		 * pc.setBaphomet(true);
		 ...
		 * pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.
		 * class), pc, 10881), true);
		 * ChattingController.toChatting(pc, "\\fS라우풀 2단계: AC-6, 마법방어+3",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * // 30000 ~ 32767
		 * if (pc.getLawful() >= 95536 && (!pc.isBaphomet() || pc.getBaphometLevel() !=
		 * 3)) {
		 * if (pc.isBaphomet())
		 * removeBaphomet(pc);
		 * pc.setBaphomet(true);
		 ...
		 * pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.
		 * class), pc, 10881), true);
		 * ChattingController.toChatting(pc, "\\fS라우풀 3단계: AC-9, 마법방어+6",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * }
		 * //카오틱 일때
		 ...
		 * // -10000 ~ -19999
		 * if (pc.getLawful() >= 45537 && pc.getLawful() <= 55536 && (!pc.isBaphomet()
		 * || pc.getBaphometLevel() != -1)) {
		 * if (pc.isBaphomet())
		 * removeBaphomet(pc);
		 * pc.setBaphomet(true);
		 ...
		 * pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.
		 * class), pc, 10879), true);
		 * ChattingController.toChatting(pc, "\\fY카오틱 1단계: 추가 대미지+1, 주술력+1",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * // -20000 ~ -29999
		 * if (pc.getLawful() >= 35537 && pc.getLawful() <= 45536 && (!pc.isBaphomet()
		 * || pc.getBaphometLevel() != -2)) {
		 * if (pc.isBaphomet())
		 * removeBaphomet(pc);
		 * pc.setBaphomet(true);
		 ...
		 * pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.
		 * class), pc, 10879), true);
		 * ChattingController.toChatting(pc, "\\fY카오틱 2단계: 추가 대미지+3, 주술력+2",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * // -30000 ~ -32768
		 * if (pc.getLawful() <= 35536 && (!pc.isBaphomet() || pc.getBaphometLevel() !=
		 * -3)) {
		 * if (pc.isBaphomet())
		 * removeBaphomet(pc);
		 * pc.setBaphomet(true);
		 ...
		 * pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.
		 * class), pc, 10879), true);
		 * ChattingController.toChatting(pc, "\\fY카오틱 3단계: 추가 대미지+5, 주술력+3",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * }
		 ...
		 * removeBaphomet(pc);
		 * ChattingController.toChatting(pc, "\\fW라우풀 효과 종료",
		 * Lineage.CHATTING_MODE_MESSAGE);
		 * }
		 * }
		 * // 효과 제거
		 ...
		 * class), pc));
		 * pc.toSender(S_CharacterSpMr.clone(BasePacketPooling.getPool(S_CharacterSpMr.
		 * class), pc));
		 * }
		 * // 효과 부여
		...
		if (pc.getLawful() >= 75536) {
			// 1단계 (103번)
			if (pc.getLawful() >= 75536 && pc.getLawful() <= 85535
				...
				appendBaphomet(pc);
				// 아이콘 출력: 103번
				SC_BUFFICON_NOTI.on(pc, 103, -1, 0);
			...
			if (pc.getLawful() >= 85536 && pc.getLawful() <= 95535
				...
				appendBaphomet(pc);
				// 아이콘 출력: 104번
				SC_BUFFICON_NOTI.on(pc, 104, -1, 0);
				pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), pc, 10881), true);
			...
			if (pc.getLawful() >= 95536 && (!pc.isBaphomet() || pc.getBaphometLevel() != 3)) {
				...
				appendBaphomet(pc);
				// 아이콘 출력: 105번
				SC_BUFFICON_NOTI.on(pc, 105, -1, 0);
				pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), pc, 10881), true);
				ChattingController.toChatting(pc, "\\fS라우풀 3단계: AC-9, 마법방어+6", Lineage.CHATTING_MODE_MESSAGE);
			}
		}
		// 카오틱 일때
		else if (pc.getLawful() <= 55536) {
			// 1단계 (106번)
			if (pc.getLawful() >= 45537 && pc.getLawful() <= 55536
					&& (!pc.isBaphomet() || pc.getBaphometLevel() != -1)) {
				if (pc.isBaphomet())
					removeBaphomet(pc);
				pc.setBaphomet(true);
				...
				appendBaphomet(pc);
				// 아이콘 출력: 106번
				SC_BUFFICON_NOTI.on(pc, 106, -1, 0);
			...
			if (pc.getLawful() >= 35537 && pc.getLawful() <= 45536
				...
				appendBaphomet(pc);
				// 아이콘 출력: 107번
				SC_BUFFICON_NOTI.on(pc, 107, -1, 0);
				pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), pc, 10879), true);
			...
			if (pc.getLawful() <= 35536 && (!pc.isBaphomet() || pc.getBaphometLevel() != -3)) {
				...
				appendBaphomet(pc);
				// 아이콘 출력: 108번
				SC_BUFFICON_NOTI.on(pc, 108, -1, 0);
				pc.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), pc, 10879), true);
		...
		if (pc.getLawful() >= 55537 && pc.getLawful() <= 75535 && pc.isBaphomet()) {
			removeBaphomet(pc);
			pc.setBaphomet(false);
			pc.setBaphometLevel(0); // 레벨 초기화 추가
			ChattingController.toChatting(pc, "\\fW라우풀/카오틱 효과 종료", Lineage.CHATTING_MODE_MESSAGE);
		}
	}

	// 효과 제거
	public static void removeBaphomet(PcInstance pc) {
		// 모든 바포메트 관련 아이콘 제거 (103~108)
		for (int i = 103; i <= 108; i++) {
			SC_BUFFICON_NOTI.on(pc, i, 0, SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
	...
	// 효과 부여 (위의 setBaphomet에서 아이콘 패킷을 개별 전송하므로 스태틱 갱신만 수행)
	public static void appendBaphomet(PcInstance pc) {
		switch (pc.getBaphometLevel()) {
			case -1:
				pc.setDynamicAddDmg(pc.getDynamicAddDmg() + 1);
