...
public class 지하수로컨트롤러 {
    // 상태
    private static Calendar calendar;
    public  static boolean isOpen;
    public  static long    wgEndTime;
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.wg_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.wg_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
    ...
    private static void open(long nowMs) {
        isOpen = true;
        wgEndTime = nowMs + (1000L * Lineage.wg_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg   = "\\fY      ***** 지하수로로 가는길이 열렸습니다. *****";
            toastTitle = "★ 지하수로 입장 가능 ★";
            toastDesc  = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg   = "\\fY      ***** 지하수로로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 지하수로 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        int remainSec = 0;
        if (isOpen) {
            long diff = wgEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
