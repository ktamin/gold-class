...
public class 칠흑던전컨트롤러 {
    // 상태
    private static Calendar calendar;
    public  static boolean isOpen;
    public  static long    darkEndTime;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        darkEndTime = nowMs + (1000L * Lineage.dark_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = darkEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
