...
public class 마족신전컨트롤러 {
    ...
    private static void open(long nowMs) {
        isOpen = true;
        deteEndTime = nowMs + (1000L * Lineage.dete_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg = "\\fY      ***** 마족 신전으로 가는길이 열렸습니다. *****";
            toastTitle = "★ 마족 신전 입장 가능 ★";
            toastDesc = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg = "\\fY      ***** 마족 신전으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 마족 신전 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = deteEndTime - System.currentTimeMillis();
            if (diff < 0)
                diff = 0;
