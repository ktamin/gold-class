...
public class 악마왕의영토컨트롤러 {
    ...
    private static void open(long nowMs) {
        isOpen = true;
        devilEndTime = nowMs + (1000L * Lineage.devil_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void pushTimerTo(PcInstance pc) {
        int remainSec = 0;
        if (isOpen) {
            long diff = devilEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
