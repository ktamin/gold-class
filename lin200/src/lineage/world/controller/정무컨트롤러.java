...
public class 정무컨트롤러 {
    ...
    public  static long    jungmuEndTime;
    ...
    public static void toTimer(long nowMs) {
        ...
        // 열려 있으면 주기 갱신/종료 처리
        if (isOpen) {
            long diffMs   = jungmuEndTime - nowMs;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        jungmuEndTime = nowMs + (1000L * Lineage.jungmu_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = jungmuEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
