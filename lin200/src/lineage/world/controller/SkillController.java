...
//		if(isAttackMagicAction(skill.getUid())){
//			cha.lastMagicAttTime = time + Lineage.attackAndMagic_delay;
//			ChattingController.toChatting(cha, ""+skill.getSkillNumber(), Lineage.CHATTING_MODE_MESSAGE);
//		}
		...
		// 실패.
		//ChattingController.toChatting(cha, "\\fY마법 재사용시간: "+cha.magic_time/1000+"초", Lineage.CHATTING_MODE_MESSAGE);
		...
		if (skill != null && isDelay(cha, skill)) {
			...
			case 3:
				...
				case 1:
					...
					break;
				case 2:
					ExtraHeal.init(cha, skill, cbp.readD());
					break;
				case 3:
			...
			case 6:
				...
				case 6:
					...
					ArmorBreak.init(cha, skill, cbp.readD());
					break;
				case 7:
			...
			case 7:
				...
				case 1:
					...
					break;
				case 2:
					SummonMonster.init(cha, skill, cbp.readH());
					break;
				case 3:
			...
			case 20:
				...
				case 3:
					...
					break;
				case 4:
					ExoticVitalize.init(cha, skill, cbp.readD());
					break;
				case 5:
	...
	/**
	 * 마법을 시전하기전에 hp, mp, item 및 상태에 따라 마법시전해도 되는지 확인해보는 메서드. : skill 이 null 로 올수도 잇기 때문에 gamso 값이 true일때만 사용하기.
	 */
	...
	static public boolean isFigure(Character cha, object o, Skill skill, boolean is_probability, boolean is_clan) {
		...
		if (isZoneCheckMagic(skill) && cha instanceof PcInstance && o instanceof PcInstance &&
				!World.isCombatZone(cha.getX(), cha.getY(), cha.getMap()) && !World.isCombatZone(o.getX(), o.getY(), o.getMap()) &&
		...
		if (skill.getUid() == 721) {
			...
			probability *= Lineage_Balance.armor_break;
		...
		if (isElfMagic(skill)) {
			...
			if (skill.getUid() == 130)
				probability *= Lineage_Balance.area_of_silence;
                        // 이레이즈 매직
			if (skill.getUid() == 123)
				probability *= Lineage_Balance.erase_magic;
	...
	static public int getDamage(Character cha, object target, object o, Skill skill, double alpha_dmg, int skill_element) {
		...
		if (!Lineage_Balance.is_fusion_attack && World.getMapdynamic(cha.getX(), cha.getY(), cha.getMap()) > 1 && cha instanceof PcInstance && o instanceof PcInstance 
				&& cha.getObjectId() != o.getObjectId() && !World.isBattleZone(cha.getX(), cha.getY(), cha.getMap()) && !World.isBattleZone(o.getX(), o.getY(), o.getMap())) {
		...
		if (isLawfulMagic(skill) && cha instanceof PcInstance && cha.getLawful() >= Lineage.NEUTRAL + 500 && item == null){
...
/*		try {
			if (cha.getGm() > 0) {			
				...
				System.out.println(String.format("[%s] [%s->%s] [스킬: %s] [데미지: %.1f] 마지막 공격 시간차: %d", timeString, cha.getName(), o.getName(), skill.getName(), dmg, time - cha.testTime));
				cha.testTime = time;
			}
		} catch (Exception e) {
	...
	static public int getMr(Character cha, boolean packet) {
		...
		mr = Math.round(mr);
		return Math.max(Math.min(Lineage.max_mr, mr), 0);
