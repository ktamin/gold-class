...
public class 얼던컨트롤러 {
    ...
    public static void init() {
        ...
        if (ALWAYS_OPEN) {
            ...
            isOpen = true;
            iceEndTime = Long.MAX_VALUE; // 의미 없음이지만 닫히지 않도록 매우 크게
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.ice_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.ice_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
    ...
    private static void open(long nowMs) {
        isOpen = true;
        iceEndTime = nowMs + (1000L * Lineage.ice_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg   = "\\fY      ***** 얼음 던전으로 가는길이 열렸습니다. *****";
            toastTitle = "★ 얼음 던전 입장 가능 ★";
            toastDesc  = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg   = "\\fY      ***** 얼음 던전으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 얼음 던전 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = iceEndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
