...
public class 오만6층컨트롤러 {
    // 상태
    private static Calendar calendar;
    public  static boolean isOpen;
    public  static long    oman6EndTime;
    ...
    public static void toTimer(long nowMs) {
        ...
        if (!isOpen) {
            if (day == 1 || day == 7) {
                // 주말 스케줄
                for (TeamBattleTime t : Lineage.oman6_dungeon_time_list2) {
                ...
                // 평일 스케줄
                for (TeamBattleTime t : Lineage.oman6_dungeon_time_list) {
                    if (t.getHour() == hour && t.getMin() == min) {
                        open(nowMs);
                        break;
                    }
        ...
        // 열려 있으면 주기 갱신/종료 처리
        if (isOpen) {
            long diffMs   = oman6EndTime - nowMs;
    ...
    private static void open(long nowMs) {
        isOpen = true;
        oman6EndTime = nowMs + (1000L * Lineage.oman6_play_time); // 설정된 '초'를 ms로
        lastRemainSecSent = -1;
        nextTimerBroadcastAt = 0L;
    ...
    public static void sendMessage() {
        ...
        if (isOpen) {
            chatMsg   = "\\fY      ***** 오만6층으로 가는길이 열렸습니다. *****";
            toastTitle = "★오만 6층 입장 가능 ★";
            toastDesc  = "던전이 열렸습니다. 지금 바로 입장하세요!";
        } else {
            chatMsg   = "\\fY      ***** 오만6층으로 가는길이 닫혔습니다. *****";
            toastTitle = "■ 오만 6층 닫힘 안내";
    ...
    public static void pushTimerTo(PcInstance pc) {
        ...
        if (isOpen) {
            long diff = oman6EndTime - System.currentTimeMillis();
            if (diff < 0) diff = 0;
