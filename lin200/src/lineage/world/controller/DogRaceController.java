...
public class DogRaceController implements Runnable {
	...
	// 마법인형 이름들
	static private String[] MAGIC_DOLL_NAME;
	static private String[] MENT;
	// 레이스견 gfx아이디
	...
	// 멘트를 위한 임시 변수.
	static private long temp_ment_time;
	//
		...
		while(running){
			try {
				...
				for(Racer r : list_race){
					...
					if (r.finish || r.runDelayTime > time){
					}else{
						if(r.isAi(time)){
							r.runDelayTime = 0;
							// 골인지점 도착 확인.
	...
	static public List<String> getRacerStatus() {
		...
		for (Racer s : list_race) {
			...
			list_html.add(MAGIC_DOLL_NAME[s.idx] + "    "); // 이름
			switch (s.Status) {
			case 0:
				list_html.add("나쁨    ");
				break;
	...
	static public void toTimer(long time) {
		if (Lineage.is_magic_doll_race) {
			...
			case CLEAR:
				if (temp_now_idx == 0) {
					temp_now_idx = Lineage.magic_doll_race_delay;
					// 문 닫기
					toDoor(false);
					// 레이서들 배치하기
					initRacer();
				...
				if (status == EVENT_STATUS.CLEAR && temp_ment_time <= time) {
					temp_ment_time = time + (1000 * Util.random(10, 20));
					toMessage(MENT[Util.random(0, MENT.length - 1)]);
				}
				...
				if (temp_now_time <= time) {
					...
					else if (temp_now_idx == 1)
						toMessage("표 판매가 마감되었습니다. 1분 후 경기가 시작됩니다.");
	...
	static private void initRacerTicket(){
		for(ShopInstance si : list_npc){
			for(int i=0 ; i<5 ; ++i){
				si.getNpc().getShop_list().get(i).setPrice(Lineage.magic_doll_race_price);
	...
	static private void setRacerTheory() {
		for (Racer s : list_race) {
			s.Theory = Util.random(0.0, 0.9) + Util.random(0, 99);
			s.Status = Util.random(0, 2);
		}
	}
	
	static private boolean checkLucky(Racer s) {
		if (!s.Lucky) {
			// 럭키 확률 체크.
			switch (s.Status) {
			case 0: // 나쁨
...
/*				idx = Util.random(0, DOG_NAME.length-1);
				...
				idx = Util.random(0, MAGIC_DOLL_NAME.length-1);
				name = MAGIC_DOLL_NAME[idx];
...
/*			int idx = Util.random(0, DOG_GFX.length-1);
	...
	static private int getPrice(String name){
		...
		try {
			...
			if(r!=null && r.getName().equalsIgnoreCase( list_Finish.get(0) )){
				...
				int slim_countting = r.countting * Lineage.magic_doll_race_price;						// 우승한 레이서의 표 갯수*price
				for(Racer s : list_race)
					total += s.countting;
				total *= Lineage.magic_doll_race_price;
	...
	static private double getRate(String name){
		...
		try {
			...
			if(r!=null){
				...
				rate = rate / Lineage.magic_doll_race_price;
			}
		} catch (Exception e) { }
		return rate;
	}
	...
	static private boolean isLucky(Racer s) {
		...
		if (!s.Lucky && Util.random(0.0, 100.0) > s.Theory) {
			switch (s.Status) {
			case 0: // 나쁨
				return Util.random(0, 100) < 80;
			case 1: // 보통
	...
	static private void isLucky2(Racer r, long time) {
		if (!checkLucky(r) && r.runDelayTime == 0)
			r.runDelayTime = time + (Util.random(0.0, 100.0) < r.Theory ? 0: SpriteFrameDatabase.getGfxFrameTime(r, r.getGfx(), r.getGfxMode()) / 2);
	}
	...
	static private void toMessage(String msg){
		for (ShopInstance si : list_npc) {
			if (si.getNpc().getNameId().equalsIgnoreCase("$858"))
