...
import Fx.server.MJTemplate.MJProto.Models.SC_BUFFICON_NOTI; // 아이콘 패킷 추가
...
public class MagicDollInstance extends object {
	...
	static synchronized public MagicDollInstance clone(MagicDollInstance mdi, Doll doll, MagicdollList mdl) {
		...
		int duration = mdl.getDollContinuous(); // 소환 유지 시간(초)
		mdi.time_end = mdi.time_start + (1000 * duration);
		...
		// [추가] 인형 소환 시 아이콘 출력 (33번)
		if (doll.getMaster() instanceof PcInstance) {
			PcInstance pc = (PcInstance) doll.getMaster();
			SC_BUFFICON_NOTI.on(pc, 33, duration, SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
	...
	private String dollBuffType;
	
	public String getDollBuffType() { return dollBuffType; }
	public void setDollBuffType(String dollBuffType) { this.dollBuffType = dollBuffType; }
	...
	@Override
	public void close() {
		// [추가] 인형 소환 해제 시 아이콘 삭제 (33번)
		if (doll != null && doll.getMaster() instanceof PcInstance) {
			PcInstance pc = (PcInstance) doll.getMaster();
			SC_BUFFICON_NOTI.on(pc, 33, 0, SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
	...
	protected void toAiWalk(long time) {
		...
		// 주인 따라다니기 로직
		if (doll != null && doll.getMaster() != null && !Util.isDistance(this, doll.getMaster(), Lineage.magicdoll_location)) {
			aStar.cleanTail();
