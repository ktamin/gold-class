...
public class NpcInstance extends Character {
	...
	// NpcInstance.java (또는 상위 공통 클래스)
	private boolean blockRespawn = false;   // 죽으면 절대 다시 스폰하지 않음
	private boolean noTransform  = false;   // (선택) 변신/체인 스폰 금지

	public boolean isBlockRespawn() { return blockRespawn; }
	public void setBlockRespawn(boolean v) { blockRespawn = v; }

	public boolean isNoTransform() { return noTransform; }
	public void setNoTransform(boolean v) { noTransform = v; }

	private static final java.util.Set<Long> __processedDeadIds =
	...
	protected void toAiDead(long time){
	    ...
	    long oid = getObjectId();
	    if (!__processedDeadIds.add(oid)) {
	        // 이미 사망 처리를 한 번 했다면 스폰 로직을 전혀 타지 않게
	        super.toAiDead(time);
	        return;
	    }
...
//		super.toAiDead(time);
	    ...
	    // ★ 스폰으로 만든 몹(상자/악령 등)은 변신/스폰 로직 자체를 건너뛰기
	    String __n = getName();
	    if (__n.contains("깜짝") || __n.contains("보너스") || __n.contains("악령")) {
	        // 상자/악령은 여기서 기본 사망 처리만 하게 하고 스폰 로직은 타지 않음
	        super.toAiDead(time);
	        return;
	    }
