...
public class ShopInstance extends object {
     ...
     */
    public int getTax() {
        if (Lineage.shop_no_tax_npc != null && getNpc() != null
                && Lineage.shop_no_tax_npc.contains(getNpc().getName())) {
            return 0;
        }
            ...
            for (Shop s : npc.getShop_list()) {
                ...
                if (s.isItemSell()) {
                    ...
                    for (ItemInstance item : search_list) {
                        /*
                         * if (!item.isEquipped() && item.getItem().isSell() && (s.getItemEnLevel() == 0
                         * || s.getItemEnLevel() == item.getEnLevel())) {
                         * //
                         * if (isSellAdd(item) && !sell_list.contains(item) && item.getEnLevel() ==
                         * s.getItemEnLevel())
                         * sell_list.add(item);
                         */
//                        if (!item.isEquipped() && item.getItem().isSell()
//                                && (s.getItemEnLevel() == item.getEnLevel() && s.getItemBress() == item.getBless())) {
//                            
//                            if (isSellAdd(item) && !sell_list.contains(item))
//                                sell_list.add(item);
                    	// ✅ 판매목록 필터(인챈/축복 인식)
                    	// - s.getItemEnLevel() == 0 이면 인챈 무관 허용(원하면 제거 가능)
                    	// - s.getItemBress() == -1 이면 축복 무관 허용(원하면 제거 가능)
                    	if (!item.isEquipped() && item.getItem().isSell()) {

                    	    boolean enOk    = (s.getItemEnLevel() == 0) || (s.getItemEnLevel() == item.getEnLevel());
                    	    boolean blessOk = (s.getItemBress() == -1) || (s.getItemBress() == item.getBless());
    ...
    protected void toBuy(PcInstance pc, ClientBasePacket cbp) {
        ...
        if (count > 0 && count <= 100) {
            ...
            for (int j = 0; j < count; ++j) {
                ...
                ItemInstance temp1 = pc.getInventory().value(item_idx);

                // //야도란 상점버그 방지추가
                // if (item_idx != temp1.getObjectId()) {
                ...
                // return;
                // }
                // if (item_count > temp1.getCount()) {
                // item_count = temp1.getCount();
                // }
                //
                // if (temp1.getCount() < item_count || temp1.getCount() <= 0 || item_count <=
                // 0) {
                // pc.toSender(S_Disconnect.clone(BasePacketPooling.getPool(S_Disconnect.class),
                // 0x0A));
                //
                ...
                if (item_count > 0 && item_count <= 1000) {
                    ...
                    if (s != null) {
                        ...
                        if (s.getPrice() != 0) {
                            ...
                            if ((i.getType1().equalsIgnoreCase("weapon") || i.getType1().equalsIgnoreCase("armor"))
                                ...
                                if ((i.getName().equalsIgnoreCase(Lineage.scroll_dane_fools)
                                    ...
                                    shop_price = getTaxPrice(s.getItemBress() == 0
                                            ? ItemDatabase.find(i.getNameIdNumber()).getShopPrice()
                                                    * Lineage.sell_bless_item_rate
                                            : ItemDatabase.find(i.getNameIdNumber()).getShopPrice()
                                                    * Lineage.sell_curse_item_rate,
                                            false);
                                else
                        ...
                        if (pc.getInventory().isAppend(i, count, i.isPiles() ? 1 : new_item_count)) {
                            ...
                            if (s.getAdenType().equalsIgnoreCase("마일리지")) {
                                ...
                                if (check1 > check2) {
                                    ...
                                    ItemInstance temp = pc.getInventory().find(s.getItemName(), s.getItemBress(),
                                            i.isPiles());
                                    if (temp == null) {
                                        ...
                                        if (i.isPiles()) {
                                            ...
                                            if (i.getName().equalsIgnoreCase("신성한 엘름의 축복"))
                                                temp.setDynamicMr(s.getItemEnLevel() > 4
                                                        ? (s.getItemEnLevel() - 4) * i.getEnchantMr()
                                                        : 0);
                                            else
                                            ...
                                            temp.setDynamicIgnoreReduction(
                                                    s.getItemEnLevel() * i.getEnchantIgnoreReduction());
                                            temp.setDynamicSwordCritical(
                                                    s.getItemEnLevel() * i.getEnchantSwordCritical());
                                            temp.setDynamicBowCritical(s.getItemEnLevel() * i.getEnchantBowCritical());
                                            temp.setDynamicMagicCritical(
                                                    s.getItemEnLevel() * i.getEnchantMagicCritical());
                                            temp.setDynamicPvpDmg(s.getItemEnLevel() * i.getEnchantPvpDamage());
                                            temp.setDynamicPvpReduction(
                                                    s.getItemEnLevel() * i.getEnchantPvpReduction());
                                            ...
                                            for (int k = 0; k < new_item_count; ++k) {
                                                ...
                                                temp.setDynamicSwordCritical(
                                                        s.getItemEnLevel() * i.getEnchantSwordCritical());
                                                temp.setDynamicBowCritical(
                                                        s.getItemEnLevel() * i.getEnchantBowCritical());
                                                temp.setDynamicMagicCritical(
                                                        s.getItemEnLevel() * i.getEnchantMagicCritical());
                                                temp.setDynamicPvpDmg(s.getItemEnLevel() * i.getEnchantPvpDamage());
                                                temp.setDynamicPvpReduction(
            ...
            try {
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    if (temp != null && !temp.isEquipped() && item_count > 0 && temp.getCount() >= item_count) {
                        ...
                        long aden_objid = 0;
                        //
                        Shop s = npc.findShopItemId2(temp.getItem().getName(), temp.getBless(), temp.getEnLevel());
                        // 판매될수 있는 아이템만 처리.
                        if (s != null && s.isItemSell()) {
                            ...
                            if (target_price > 0) {
                                ...
                                Log.appendItem(pc, "type|상점판매금", "npc_name|" + getNpc().getName(),
                                        ...
                                        "target_objid|" + target_objid, "target_price|" + target_price,
    ...
    public int getPrice(Connection con, ItemInstance item) {
     ...
     // ✅ 인챈까지 포함해서 상점 테이블에서 가격 찾기
        Shop shop = npc.findShopItemId2(item.getItem().getName(), item.getBless(), item.getEnLevel());

//        Shop shop = npc.findShopItemId(item.getItem().getName(), item.getBless());
        // 그외 일반 아이템 가격 추출.
        if (item == null || shop == null/* || item.getItem().getType2().equalsIgnoreCase("arrow") */) {
            ...
            if (shop.getPrice() != 0) {
                ...
                if ((item.getItem().getName().equalsIgnoreCase(Lineage.scroll_dane_fools)
                    ...
                    return getTaxPrice(item.getItem().getShopPrice()
                            * (item.getBless() == 0 ? Lineage.sell_bless_item_rate : Lineage.sell_curse_item_rate),
                            true);
                else
