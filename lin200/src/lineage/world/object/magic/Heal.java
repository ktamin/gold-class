...
public class Heal {
	...
	static public void init(Character cha, Skill skill, long object_id) {
		...
		if (o != null) {
			...
			if (cha instanceof PcInstance) {
				...
				SC_SKILL_DELAY_NOTI.newInstance().setDurationMs(1100).send(pc);
			}
	...
	static public void onBuff(Character cha, object o, Skill skill, int effect, int alpha_dmg) {
		...
		if (skill != null && !Lineage_Balance.is_heal_damage && cha instanceof PcInstance && o instanceof PcInstance
				&& (skill.getUid() == 49 || skill.getUid() == 133)) {
	...
	public static int getHealDamage(Character cha, object o, Skill skill) {
		...
		if (o instanceof MonsterInstance
				&& !(o.getSummon() != null && o.getSummon().getMasterObjectId() == cha.getObjectId())) {
			...
			if (mon.getMonster().isUndead() && mon.getMonster().isTurnUndead()) {
				...
				if (SpriteFrameDatabase.findGfxMode(mon.getGfx(), mon.getGfxMode() + Lineage.GFX_MODE_DAMAGE))
					mon.toSender(S_ObjectAction.clone(BasePacketPooling.getPool(S_ObjectAction.class), mon,
							Lineage.GFX_MODE_DAMAGE), false);
				return 0;
			}
		}
