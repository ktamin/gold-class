...
public class SoulOfFlame extends Magic {
	...
	public void toBuffStart(object o) {
		// 1. 상태 설정 (최대 대미지 및 무기 손상 방지 로직은 타격 연산에서 이 플래그를 체크함)
	...
	public void toBuffUpdate(object o) {
		// 2. 버프 갱신 및 아이콘 유지 (25번)
		// 기존에 반복되던 이펙트(S_ObjectEffect)는 제거하고 아이콘 패킷으로 교체
		if (o instanceof PcInstance) {
			PcInstance pc = (PcInstance) o;
			SC_BUFFICON_NOTI.on(pc, 25, this.getTime(), SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
	}
	...
	public void toBuffEnd(object o) {
		...
		// 4. 종료 시 아이콘 즉시 삭제
		if (o instanceof PcInstance) {
			SC_BUFFICON_NOTI.on((PcInstance)o, 25, 0, SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
		
		// 종료 멘트 중복 방지 (주석 처리)
		// ChattingController.toChatting(o, "\\fY소울 오브 프레임 종료", Lineage.CHATTING_MODE_MESSAGE);
	}
	...
	public void toBuff(object o) {
		...
		// 	ChattingController.toChatting(o, "\\fY소울 오브 프레임: " + getTime() + "초 후 종료", Lineage.CHATTING_MODE_MESSAGE);
	}
	...
	static public void init(Character cha, Skill skill) {
		...
		if (cha.getInventory().getSlot(Lineage.SLOT_WEAPON) != null &&( cha.getInventory().getSlot(Lineage.SLOT_WEAPON).getItem().getType2().equalsIgnoreCase("sword") || cha.getInventory().getSlot(Lineage.SLOT_WEAPON).getItem().getType2().equalsIgnoreCase("dagger"))) {
			...
			if (SkillController.isMagic(cha, skill, true)) {
				...
				// 시작 멘트 중복 방지 (주석 처리)
				// ChattingController.toChatting(cha, "소울 오브 프레임: 근거리 무기의 최대 대미지로 공격, 무기 비손상", Lineage.CHATTING_MODE_MESSAGE);
			}
