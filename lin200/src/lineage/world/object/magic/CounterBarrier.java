...
public class CounterBarrier extends Magic {
	...
	public void toBuffUpdate(object o) {	
		...
		if (o instanceof PcInstance) {
			...
			SC_BUFFICON_NOTI.on(pc, 24, this.getTime(), SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
	}
	...
	public void toBuffEnd(object o) {
		...
		// 종료 시 아이콘 즉시 삭제
		if (o instanceof PcInstance) {
			SC_BUFFICON_NOTI.on((PcInstance)o, 24, 0, SC_BUFFICON_NOTI.REMAINING_TYPE_SECONDS);
		}
	...
	static public void init(Character cha, Skill skill) {
		...
		if (cha.getInventory().getSlot(Lineage.SLOT_WEAPON) != null && cha.getInventory().getSlot(Lineage.SLOT_WEAPON).getItem().getType2().equalsIgnoreCase("tohandsword")
			...
			// 실패 멘트는 유지 (시스템 메시지가 아니라 에러 메시지이므로)
			...
			else if (!cha.getInventory().getSlot(Lineage.SLOT_WEAPON).getItem().getType2().equalsIgnoreCase("tohandsword"))
