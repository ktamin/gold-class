...
public class Firewall extends Magic {
	...
	public void toBuffStart(object o) {
		synchronized (list) {
			...
			while ((nx != x || ny != y) && --cnt >= 0) {
				...
				BackgroundInstance fire = lineage.world.object.npc.background.Firewall
						.clone(BackgroundDatabase.getPool(lineage.world.object.npc.background.Firewall.class));
			...
			synchronized (list) {
				for (BackgroundInstance bi : list) {
					for (object oo : bi.getInsideList()) {
						if (!oo.isDead() && cha.getObjectId() != oo.getObjectId() && oo instanceof Character
								&& oo.getX() == bi.getX() && oo.getY() == bi.getY()) {
							// 자신의 소환수는 무시.
							if (oo.getSummon() != null && oo.getSummon().getMasterObjectId() == cha.getObjectId())
								continue;
							// 데미지 처리
							DamageController.toDamage(cha, oo,
									SkillController.getDamage(cha, oo, oo, skill, 0, skill.getElement()),
									Lineage.ATTACK_TYPE_MAGIC);
						}
					}
				}
			}
	...
	static public void init(Character cha, Skill skill, int x, int y) {
		...
		int checkX = cha.getX();
		int checkY = cha.getY();
		int mapId = cha.getMap(); // ★수정: getMapId() -> getMap() 으로 변경
		int loopCount = 20; // 최대 검사 거리 (무한 루프 방지용)

		// 시전자부터 목표 지점(x, y)까지 한 칸씩 가보면서 벽이 있는지 확인
		while ((checkX != x || checkY != y) && --loopCount >= 0) {
			int h = Util.calcheading(checkX, checkY, x, y);
			if (!World.isThroughAttack(checkX, checkY, mapId, h)) {
				// 벽에 막혔다면 즉시 종료 (모션 X, 쿨타임 X)
				return;
			}
			checkX += Util.getXY(h, true);
			checkY += Util.getXY(h, false);
		}
		...
		SC_SKILL_DELAY_NOTI.newInstance().setDurationMs(900).send((PcInstance) cha);
		if (SkillController.isMagic(cha, skill, true)) {
			BuffController.remove(cha, Firewall.class);
			BuffController.append(cha,
					Firewall.clone(BuffController.getPool(Firewall.class), skill, skill.getBuffDuration(), x, y));
		}
	}
}
