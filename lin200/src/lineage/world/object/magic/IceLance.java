...
public class IceLance extends Magic {
	...
	static public void init(Character cha, Skill skill, int object_id) {
		...
		if (o != null && Util.isDistance(cha, o, 6) && SkillController.isMagic(cha, skill, true)) {
			...
			cha.toSender(S_ObjectAttack.clone(BasePacketPooling.getPool(S_ObjectAttack.class), cha, o,
					Lineage.GFX_MODE_SPELL_DIRECTION, dmg, skill.getCastGfx(), false, false, 0, 0), true);

			// 얼리기 처리
			if (dmg > 0 && SkillController.isFigure(cha, o, skill, true, false))
				BuffController.append(o,
						IceLance.clone(BuffController.getPool(IceLance.class), skill, skill.getBuffDuration()));
		}
	}

	static public void init(object o, int time) {
		BuffController.append(o,
				IceLance.clone(BuffController.getPool(IceLance.class), SkillDatabase.find(7, 1), time));
	}
	...
	static public void init(MonsterInstance mi, object target, MonsterSkill ms) {
		if (ms.getSkill() != null) {
			if (target != null && Util.isDistance(mi, target, ms.getDistance() > 0 ? ms.getDistance() : 6)
					&& SkillController.isMagic(mi, ms, true)) {
				// 데미지 처리
				int dmg = SkillController.getDamage(mi, target, target, ms.getSkill(),
						Util.random(ms.getMindmg(), ms.getMaxdmg()), ms.getSkill().getElement());
				DamageController.toDamage(mi, target, dmg, Lineage.ATTACK_TYPE_MAGIC);

				if (dmg > 0) {
					target.toSender(S_ObjectAction.clone(BasePacketPooling.getPool(S_ObjectAction.class), target,
							Lineage.GFX_MODE_DAMAGE), true);

					// 얼리기 처리
					if (SkillController.isFigure(mi, target, ms.getSkill(), true, false))
						BuffController.append(target, IceLance.clone(BuffController.getPool(IceLance.class),
								ms.getSkill(), ms.getSkill().getBuffDuration()));
				}
			}
		}
	}
