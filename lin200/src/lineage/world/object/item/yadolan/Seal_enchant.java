...
public class Seal_enchant extends ItemInstance {
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		...
		if (!target.getItem().getName().equalsIgnoreCase("수호의인장")) {
			ChattingController.toChatting(cha, "수호의인장 이외의 아이템은 인첸트 할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
			return;
		}
		...
		if (Util.random(1, 100) < chance) {
			...
			ChattingController.toChatting(cha, "수호의인장 강화에 성공하였습니다.", Lineage.CHATTING_MODE_MESSAGE);
			...
			ChattingController.toChatting(cha, "수호의인장 강화에 실패하여 증발 하였습니다.", Lineage.CHATTING_MODE_MESSAGE);
