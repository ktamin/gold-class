...
public class ScrollOfmythRankPoly extends ItemInstance {
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		if (!cha.isDead() && !cha.isWorldDelete() && cha.getInventory() != null && cha instanceof PcInstance) {
			...
			if(cha.getMap() == 807){
				ChattingController.toChatting(this, "여기서는 사용 하실 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
				return;
			}
			// 팀대전, 낚시중에 변신 불가.
			if (!World.isTeamBattleMap(cha) && !cha.isFishing()) {
				...
				case 4:
					if (cha.getClassSex() == 0)
						polyName = "남자다엘 랭커 야도란";
					else
						polyName = "여자다엘 랭커 야도란";
					break;	
				}
				...
				if (p != null) {
					...
					//야도란 신화변신 퀵변신에 추가 보류
//					cha.setQuickPolymorph(polyName);
					...
					// 아이템 수량 갱신
					if (!getItem().getName().contains("반지") && !getItem().getName().contains("카드")) {
