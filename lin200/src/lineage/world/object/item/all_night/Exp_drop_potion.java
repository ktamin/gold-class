...
public class Exp_drop_potion extends ItemInstance {
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		if (cha.getInventory() != null && getItem() != null) {
			...
			if (getItem().getType2().equalsIgnoreCase("경험치 물약 10") || getItem().getType2().equalsIgnoreCase("무한 경험치 물약 10")) {
				if (checkBuff(cha, 701))
					ExpDropBuff_10.init(cha, SkillDatabase.find(701));
				else
					return;
			} else if (getItem().getType2().equalsIgnoreCase("경험치 물약 20") || getItem().getType2().equalsIgnoreCase("무한 경험치 물약 20")) {
				if (checkBuff(cha, 702))
					ExpDropBuff_20.init(cha, SkillDatabase.find(702));
				else
					return;
			} else if (getItem().getType2().equalsIgnoreCase("경험치 물약 50") || getItem().getType2().equalsIgnoreCase("무한 경험치 물약 50")) {
			...
			if (!getItem().getType2().contains("무한 경험치 물약"))
