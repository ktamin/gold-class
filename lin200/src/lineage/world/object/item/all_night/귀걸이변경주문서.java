...
public class 귀걸이변경주문서 extends ItemInstance {
    ...
    static synchronized public ItemInstance clone(ItemInstance item) {
        if (item == null) item = new 귀걸이변경주문서();
        return item;
    }
    ...
    public void toClick(Character cha, ClientBasePacket cbp) {
        ...
        if (src == null || !(src instanceof ItemArmorInstance)) {
            ChattingController.toChatting(cha, "귀걸이에 사용 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
            return;
        }
        if (src.getItem() == null) {
        ...
        if (containsIgnoreCase(GRADE1, srcName)) {
            ...
            ChattingController.toChatting(cha, "해당 귀걸이에 사용할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
            return;
        }
