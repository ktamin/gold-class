						...
						if (item != null) {
							...
							if (재료 != null) {
								for (ItemInstance i : pc.getInventory().getList()) {
									if (i != null && i.getItem() != null && !i.isEquipped()
											&& i.getItem().getName().equalsIgnoreCase(재료) && i.getCount() >= 재료수량) {
										제거할아이템 = i;
										break;
									}
								}
							...
							if (제거할아이템 != null) {
								if (EnchantLostItemDatabase.deleteDB(el)) {
									...
									ItemInstance temp = pc.getInventory().find(item.getName(), el.getBless(),
											item.isPiles());

									if (temp != null && (temp.getBless() != el.getBless()
											|| temp.getEnLevel() != el.getEn_level()))
										temp = null;
...
//											temp.setEnFire(el.getEnFire());
//											temp.setEnWater(el.getEnWater());
//											temp.setEnWind(el.getEnWind());
//											temp.setEnEarth(el.getEnEarth());
											if (el.getEnFire() > 0) {
											    temp.setEnFire(el.getEnFire());
											} else if (el.getEnWater() > 0) {
											    temp.setEnWater(el.getEnWater());
											} else if (el.getEnWind() > 0) {
											    temp.setEnWind(el.getEnWind());
											} else if (el.getEnEarth() > 0) {
											    temp.setEnEarth(el.getEnEarth());
											}
...
//												temp.setEnEarth(el.getEnEarth());
									...
									ChattingController.toChatting(pc, String.format("\\fR'%s' 복구 완료!", msg),
									...
									if (Lineage.is_recovery_scroll && el.getScroll_name() != null) {
										...
										if (scroll != null) {
											ItemInstance temp_scroll = pc.getInventory().find(scroll.getName(),
													el.getScroll_bless(), scroll.isPiles());
											...
											ChattingController.toChatting(pc,
													String.format("\\fR'%s' 복구 완료!", getScrollName(el)),
													Lineage.CHATTING_MODE_MESSAGE);
										}
									}
									...
									if (!Common.system_config_console) {
										...
										String log = String.format(
												"[%s]\t [캐릭터: %s]\t [아이템: %s]\t [주문서: %s]\t [잃은시간: %s]", timeString,
												pc.getName(), msg, getScrollName(el), lostTime);
								...
								ChattingController.toChatting(pc,
										String.format("\\fR[아이템 복구] \\fY%s(%d) \\fR이(가) 필요합니다.", 재료, 재료수량),
										Lineage.CHATTING_MODE_MESSAGE);
							}
						}
					}
	...
	public String getScrollName(EnchantLostItem el) {
		if (el.getScroll_name() != null) {
			return String.format("%s%s", el.getScroll_bless() == 1 ? "" : el.getScroll_bless() == 0 ? "(축)" : "(저주)",
					el.getScroll_name());
		} else {
			return "";
		}
