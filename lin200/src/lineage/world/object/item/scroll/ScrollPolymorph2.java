...
public class ScrollPolymorph2 extends ItemInstance {

	static synchronized public ItemInstance clone(ItemInstance item) {
		if (item == null)
			item = new ScrollPolymorph2();
		return item;
	}
		...
		if (!cha.isFishing()) {
			...
			if (Lineage.is_rank_poly) {
				if ((((allRank > 0 && allRank <= Lineage.rank_poly_all) || (classRank > 0 && classRank <= Lineage.rank_poly_class)) && cha.getLevel() >= Lineage.rank_min_level) ||
					...
					cha.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), cha, "monlist100", null, quickPolymorph));
				else
					cha.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), cha, "monlist100", null, quickPolymorph));
			} else {
				cha.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), cha, "monlist100", null, quickPolymorph));
