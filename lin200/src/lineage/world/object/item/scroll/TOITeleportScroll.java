...
public class TOITeleportScroll extends ItemInstance {
	...
	public void toClick(Character cha, ClientBasePacket cbp){
		...
		if (cha.getLevel() < Lineage.oman_min_level) {
			ChattingController.toChatting(cha, String.format("오만의 탑은 %d레벨 이상부터 입장 가능합니다.", Lineage.oman_min_level), Lineage.CHATTING_MODE_MESSAGE);
			return;
		}
		...
		if(LocationController.isTeleportZone(cha, true, !oman) || oman){
			...
			switch(floor){
				...
				case 10:
					...
					break;
				default:
					if ((cha.getClanId() == 0 && !Lineage.is_no_clan_oman_top) || (cha.getClanName().equalsIgnoreCase(Lineage.new_clan_name) && !Lineage.is_new_clan_oman_top)) {
						ChattingController.toChatting(cha, "신규혈맹 또는 혈맹이 없을경우 이동이 불가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
						return;
					}
					
					if (Lineage.is_wanted_oman_top && cha.getGm() == 0 && !WantedController.checkWantedPc(cha)) {
