...
public class ScrollTeleport extends ItemInstance {
	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		...
		try {
			...
			if (it != null) {
				...
				if (uid == 28 && k != null && k.getClanId() > 0) {
					if (k.getClanId() == cha.getClanId()) {
						...
						ChattingController.toChatting(cha, "성혈맹이 아닐경우 사용할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
					}
				} else {
			...
			lineage.share.System.printf("%s : toClick(Character cha, ClientBasePacket cbp) [아이템: %s]\r\n", ScrollTeleport.class.toString(), getItem().getName());
