...
public class Enchant extends ItemInstance {
	...
	protected boolean isEnchant(ItemInstance item) {
		if (item == null)
			return false;
                if(item.isEquipped()) {
                    ChattingController.toChatting(cha, String.format("착용하고 있는 장비는 인챈할 수 없습니다.", Lineage.item_enchant_accessory_max), Lineage.CHATTING_MODE_MESSAGE);
                    return false;
                }
	...
	protected int toEnchant(Character cha, ItemInstance item, ItemInstance accScroll) {
                ...
                if(item.isEquipped()) {
                    ChattingController.toChatting(cha, String.format("착용한 장비는 인챈할 수 없습니다.", Lineage.item_enchant_accessory_max), Lineage.CHATTING_MODE_MESSAGE);
                    return -127;
                }
		// 기본 인첸 가능여부 판단.
		if (!isEnchant(item)) {
