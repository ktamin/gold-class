...
import lineage.world.controller.TeleportProcessController;
...
public class object {
	...
	private 클래스변경권 classChangeScroll;
	private String classChangeType;

	// 인첸트 복구
	public long[] enchantRecovery;
	
	private 캐릭터저장구슬 characterMarble;
	...
	public void close() {
		...
		count = 1;
		wisdom = eva = brave = dead = fight = moving = poison = lock_low = lock_high = invis = transparent = damageMassage = isWarMessage = isMapHack = isAntarasArmor = isFafurionArmor = isLindviorArmor = isValakasArmor = isMark = false;
		BuffDecreaseWeight = BuffHolyWeapon = BuffEnchantWeapon = BuffMonsterEyeMeat = BuffCurseParalyze = BuffBlessWeapon = BuffInvisiBility = BuffImmuneToHarm = BuffDecayPotion = BuffAbsoluteBarrier = BuffGlowingAura = BuffFireWeapon = BuffWindShot = BuffEraseMagic = BuffBurningWeapon = BuffStormShot = BuffWisdom = BuffEva = BuffBlessOfFire = BuffCurseGhoul = BuffCurseFloatingEye = BuffChattingClose = BuffDisease = BuffSilence = BuffEyeOfStorm = BuffWeakness = BuffCounterMagic = BuffCriminal = BuffMeditation = BuffFogOfSleeping = BuffEnchantVenom = BuffBurningSpirit = BuffVenomResist = BuffDoubleBreak = BuffShadowFang = BuffBerserks = BuffNaturesTouch = BuffCounterBarrier = BuffCounterMirror = nameHidden = BuffBounceAttack = BuffCurseBlind = BuffExoticVitalize = BuffWaterLife = BuffElementalFire = BuffPolluteWater = BuffStrikerGale = BuffMfire = BuffSoulOfFlame = BuffAdditionalFire = BuffBraveMental = BuffBraveAvatar  = BuffBluePotion = criticalEffect = criticalMagicEffect = BuffExpPotion = false;
		BuffAdvanceSpiritHp = BuffAdvanceSpiritMp = thread_uid = lastRank = 0;
		temp_hp = temp_mp = -1;
		worldDelete = true;
		own = null;
		database_key = null;
                viewDelaySkillName = null;
	...
	public 캐릭터저장구슬 getCharacterMarble() {
		return characterMarble;
	}

	public void setCharacterMarble(캐릭터저장구슬 characterMarble) {
	...
	public 클래스변경권 getClassChangeScroll() {
		return classChangeScroll;
	}

	public void setClassChangeScroll(클래스변경권 classChangeScroll) {
	...
	public void toTeleport(final int x, final int y, final int map, final boolean effect) {
		...
		// 순회
		long delayMillis = (long)SpriteFrameDatabase.getSpeedCheckGfxFrameTime(this, getGfx(), 0);
			...
			if (isList(o)) {
				...
				if (Util.isDistance(this, o, Lineage.SEARCH_LOCATIONRANGE)) {
					...
					if (this instanceof PcInstance) {
						...
						toSender(S_ObjectAdd.clone(BasePacketPooling.getPool(S_ObjectAdd.class), o, this));
						TeleportProcessController.append(o, this, delayMillis);
					...
					if (o instanceof PcInstance) {
						...
						o.toSender(S_ObjectAdd.clone(BasePacketPooling.getPool(S_ObjectAdd.class), this, o));
						TeleportProcessController.append(this, o, delayMillis);
					}
				}
			}
		}
