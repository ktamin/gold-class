...
public class 얼던상시텔레포터 extends object {
    ...
    public void toTalk(PcInstance pc, ClientBasePacket cbp) {
        ...
        list.add(String.format("입장재료 : 아데나 %,d 개", Lineage.go_ice));

        pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "ice24tel", null, list));
    }

    @Override
    public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
        if (!action.equalsIgnoreCase("ice24_teleport"))
            return;
        ...
        // 1) 레벨 체크
        if (pc.getLevel() < Lineage.ice_level) {
        ...
        if (Lineage.ice_wanted && !WantedController.checkWantedPc(pc)) {
            ChattingController.toChatting(pc, "얼음 던전은 수배자만 입장 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
            return;
        }

        // 3) 혈맹 조건
        if (Lineage.ice_clan && pc.getClanId() <= 0) {
        ...
        if (Lineage.go_ice > 0) {
            ...
            if (pc.getInventory() == null ||
                !pc.getInventory().isAden("아데나", Lineage.go_ice, true)) {
                ChattingController.toChatting(pc,
                    String.format("입장재료가 부족합니다. 아데나 %,d만 필요합니다.", Lineage.go_ice),
                    Lineage.CHATTING_MODE_MESSAGE);
                return;
            }
        }
