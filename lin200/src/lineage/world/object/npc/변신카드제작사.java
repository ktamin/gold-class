...
public class 변신카드제작사 extends object {
    ...
    public void toTalk(PcInstance pc, ClientBasePacket cbp) {
        pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "polycardcreate"));
    }
    ...
    public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
        if (pc.getInventory() != null) {
            ...
            if (action.equalsIgnoreCase("필리아 변신 카드")) {
                createList.add(new CreateItem("변신 카드 조각", false, 1, true, 0, 100));
                createList.add(new CreateItem("아데나", false, 1, false, 0, 30000000));
                if (!checkItem(pc, createList, itemList)) return;

                createItem(pc, createList, itemList, "필리아 변신 카드", 1, 0, 1, 100);

            } else if (action.equalsIgnoreCase("발키리 변신 카드")) {
                createList.add(new CreateItem("변신 카드 조각", false, 1, false, 0, 100));
 //               createList.add(new CreateItem("신성한 조각", false, 1, false, 0, 30));
                createList.add(new CreateItem("아데나", false, 1, false, 0, 30000000));
                if (!checkItem(pc, createList, itemList)) return;

                createItem(pc, createList, itemList, "발키리 변신 카드", 1, 0, 1, 100);

            } else if (action.equalsIgnoreCase("도플갱어 변신 카드")) {
 //               createList.add(new CreateItem("변신 조종 반지", false, 1, false, 0, 1));
                createList.add(new CreateItem("변신 카드 조각", false, 1, false, 0, 100));
                createList.add(new CreateItem("아데나", false, 1, false, 0, 30000000));
                if (!checkItem(pc, createList, itemList)) return;

                createItem(pc, createList, itemList, "도플갱어 변신 카드", 1, 0, 1, 100);

            } else if (action.equalsIgnoreCase("드슬 변신 카드")) {
//                createList.add(new CreateItem("지옥의 룬", false, 1, false, 0, 1));
                createList.add(new CreateItem("변신 카드 조각", false, 1, false, 0, 100));
                createList.add(new CreateItem("아데나", false, 1, false, 0, 30000000));
                if (!checkItem(pc, createList, itemList)) return;

                createItem(pc, createList, itemList, "드슬 변신 카드", 1, 0, 1, 100);
            }
        }
    }
    ...
    public void createItem(PcInstance pc, List<CreateItem> createList, List<ItemInstance> itemList, String createItemName, int bless, int enchant, int count, int successRate) {
        ...
        if (i != null) {
            ...
            World.toSender(S_ObjectChatting.clone(BasePacketPooling.getPool(S_ObjectChatting.class), String.format("\\fR어느 아덴 용사가 %s 제작에 성공하였습니다!", createItemName)));
        }
    }
}
