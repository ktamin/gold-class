...
public class 마족신전텔레포터 extends object {
	@Override
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "detetel", null, list));
	}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
	    ...
	    if (pc.getGm() == 0 && !마족신전컨트롤러.isOpen) {
	        ChattingController.toChatting(pc, "마족신전으로 가는길이 닫혀있습니다.", Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }

	    // 3. 레벨 제한 (GM은 무시)
	    if (pc.getGm() == 0 && pc.getLevel() < Lineage.dete_level) {
	        ChattingController.toChatting(pc, String.format("마족신전은 %d레벨 이상 입장 가능합니다.", Lineage.dete_level), Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }

	    // 4. 입장료(아데나) 체크 (GM은 무시)
	    if (pc.getGm() == 0 && !pc.getInventory().isAden("아데나", Lineage.go_dete, true)) {
	        ChattingController.toChatting(pc, String.format("입장료 아데나: %,d 부족합니다.", Lineage.go_dete), Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }

	    // 5. 수배자 여부 (GM은 무시, 옵션)
	    if (pc.getGm() == 0 && Lineage.dete_wanted && !WantedController.checkWantedPc(pc)) {
	        ChattingController.toChatting(pc, "마족신전은 수배자만 입장 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }

	    // 6. 혈맹(재확인) (GM은 무시, 옵션)
	    if (pc.getGm() == 0 && Lineage.dete_clan && pc.getClanId() == 0) {
	        ChattingController.toChatting(pc, "마족신전은 혈맹 가입자만 입장 가능합니다.", Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }
