...
import lineage.world.object.npc.용갑옷제작사.CreateItem;

public class 멸마의갑옷제작사 extends object {
    ...
    public void toTalk(PcInstance pc, ClientBasePacket cbp) {
        pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "armorcreate1"));
    }
        ...
        for (CreateItem list : createList) {
            ...
            for (ItemInstance i : pc.getInventory().getList()) {
                if (i.getItem() != null
                    ...
                    && !i.isEquipped()
                    // ★ 축복 검사: isCheckBless가 true일 때만 bless 비교
                    && (!list.isCheckBless || i.getBless() == list.bless)
                    // ★ 인챈트 검사: isCheckEnchant가 true일 때만 enchant 비교
                    && (!list.isCheckEnchant || i.getEnLevel() == list.enchant)
                ) {
                    itemList.add(i);
                    found = true;
                    break;
