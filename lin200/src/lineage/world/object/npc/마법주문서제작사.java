...
import lineage.world.object.npc.지배악세제작사.CreateItem;

public class 마법주문서제작사 extends object {
    ...
    public void toTalk(PcInstance pc, ClientBasePacket cbp) {
        pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "scrollCreate1"));
    }
    ...
    public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
        if (pc.getInventory() != null) {
            ...
            if (action.equalsIgnoreCase("희귀 제작 비법서") && !Lineage.is_gmlrnl) {
                ChattingController.toChatting(pc, "현재는 희귀 제작 비법서를 제작할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
                return;
            }
            if (action.equalsIgnoreCase("영웅 제작 비법서") && !Lineage.is_duddnd) {
                ChattingController.toChatting(pc, "현재는 영웅 제작 비법서를 제작할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
                return;
            }
            if (action.equalsIgnoreCase("전설 제작 비법서") && !Lineage.is_wjstjf) {
                ChattingController.toChatting(pc, "현재는 전설 제작 비법서를 제작할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
                return;
            }
            if (action.equalsIgnoreCase("신화 제작 비법서") && !Lineage.is_tlsghk) {
                ChattingController.toChatting(pc, "현재는 신화 제작 비법서을 제작할 수 없습니다.", Lineage.CHATTING_MODE_MESSAGE);
                return;
            }
...
//              createList.add(new CreateItem("아데나", false, 1, false, 0, 100000000));
                ...
                createItem(pc, createList, itemList, "영웅 제작 비법서", 1, 0, 1, 100);
...
//              createList.add(new CreateItem("아데나", false, 1, false, 0, 50000000));
                ...
                createItem(pc, createList, itemList, "신화 제작 비법서", 1, 0, 1, 100);
            }
        }
    }
    ...
    public void createItem(PcInstance pc, List<CreateItem> createList, List<ItemInstance> itemList, String createItemName, int bless, int enchant, int count, int successRate) {
        ...
        if (!isSuccess) {
            ChattingController.toChatting(pc, String.format("'%s' 제작에 실패하였습니다. 모든 재료가 차감되었습니다.", createItemName), Lineage.CHATTING_MODE_MESSAGE);
            return;
        }
