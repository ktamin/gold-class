...
public class 칠흑던전텔레포터 extends object {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		if(nowday == 1 || nowday == 7){
			list.add(String.format("입장 시간: %s", Lineage.dark_dungeon_time2));	
		}else{
			list.add(String.format("입장 시간: %s", Lineage.dark_dungeon_time));
		}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
	    ...
	    if (pc.getGm() == 0 && pc.getLevel() < Lineage.dark_level) {
	        ChattingController.toChatting(pc, String.format("칠흑던전은 %d레벨 이상 입장 가능합니다.", Lineage.dark_level), Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }

	    // 3. 사냥터 오픈 여부
	    if (pc.getGm() == 0 && !칠흑던전컨트롤러.isOpen) {
	        ChattingController.toChatting(pc, "칠흑 던전으로 가는길이 닫혀있습니다.", Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }
